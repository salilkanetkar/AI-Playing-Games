...
//------------------------------------------------------------------------
DECLARE_DEFAULT_COMPONENT_FACTORY(CVehicleMountedWeapon, CVehicleMountedWeapon)
	...
	if(pVehicle)
	{
		...
		if (pOwner && pOwner->IsPlayer())
		{
			...
			if(pPlayer->IsClient())
			{
				...
				float xAdjustment = (float)fsel(viewAngles.x, max(-viewAngles.x, -0.5f * timeStep), min(-viewAngles.x, 0.5f * timeStep));
				float yAdjustment = (float)fsel(viewAngles.y, max(-viewAngles.y, -0.5f * timeStep), min(-viewAngles.y, 0.5f * timeStep));

...
void CPlayerMovementController::ApplyControllerAcceleration( float& xRot, float& zRot, float dt )
{
	...
	if(g_pGameCVars->pl_aim_acceleration_enabled)
	{
		...
		float timeAtHighDeflectionX = min((float)fsel(fabsf(xRot) - 0.95f, m_timeTurningInSameDirX + dt, 0.0f), kTimeToFullMultiplier);
		float timeAtHighDeflectionZ = min((float)fsel(fabsf(zRot) - 0.95f, m_timeTurningInSameDirZ + dt, 0.0f), kTimeToFullMultiplier);
		...
		if (distance > 0.01f)
		{
			...
			if (!(pAnimTarget && (pAnimTarget->activated || m_animTargetSpeedCounter) && m_animTargetSpeed >= 0.0f))
			{
				...
				if (pAnimTarget && pAnimTarget->preparing)  
				{
					...
					float approachSpeed = frameTime == 0.0f ? 0.0f : (distance * fres(frameTime)) * 0.4f;

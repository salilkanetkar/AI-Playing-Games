...
void ScreenLayoutManager::UpdateHUDCanvasSize( void )
{
	...
	INV_RENDER_SCREEN_WIDTH = 1.0f*fres(RENDER_SCREEN_WIDTH);
	INV_RENDER_SCREEN_HEIGHT = 1.0f*fres(RENDER_SCREEN_HEIGHT);

	// Force update of HUDAssets and other objects.
	const float pa = gEnv->pRenderer->GetPixelAspectRatio();
	Vec2 curCanvasSize( RENDER_SCREEN_WIDTH*fres(pa), RENDER_SCREEN_HEIGHT );
#if 0

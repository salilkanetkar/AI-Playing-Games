...
void CUIHUD3D::InitEventSystem()
{
	...
	if(pFlashUI)
		pFlashUI->RegisterModule(this, "CUIHUD3D");
...
void CUIHUD3D::UpdateView(const SViewParams &viewParams)
{
	...
	if (m_pHUDRootEntity && pLocalPlayer)
	{
		...
		{
			deltaHudRotation.w *= (float)fres(pCVars->hud_bobHud);
		...
		{
			offsetScale = (pCVars->hud_cgf_positionScaleFOV * fres(viewParams.fov )) + distance;
		}
...
void CUIHUD3D::OnVisCVarChange( ICVar * )
{
	...
	if (pFlashUI)
		pFlashUI->SetHudElementsVisible(bVisible);
}

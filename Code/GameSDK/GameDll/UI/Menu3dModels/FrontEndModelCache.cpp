...
CFrontEndModelCache::CFrontEndModelCache()
{
	...
	if(pRootNode)
	{
		...
		if(pPakData && pPakData->getChildCount())
		{
			...
			if(pPak)
			{
				...
				bool bSucceeded = gEnv->pCryPak->OpenPack("@assets@",pPakName,ICryPak::FLAGS_FILENAMES_AS_CRC32);
				bSucceeded |= gEnv->pCryPak->LoadPakToMemory(pPakName,ICryPak::eInMemoryPakLocale_GPU);
				FE_LOG ("%s to open pack file '%s' bound to assets", bSucceeded ? "Managed" : "Failed", pPakName);
			}
	...
	if (g_pGameCVars->g_FEMenuCacheSaveList)
	{
		...
		string sResourceSetFilename = PathUtil::AddSlash("@devroot@/Levels/Multiplayer") + "mpmenu_list.txt";
		{
...
//--------------------------------------------------------------------------------------------------
#if FEMC_CACHE_FILE_ACCESSES
void CFrontEndModelCache::ReportFileOpen( AZ::IO::HandleType filehandle, const char *pFullPath )
{
	(void)filehandle;

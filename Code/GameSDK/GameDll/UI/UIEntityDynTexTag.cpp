...
//	assert(pFlashUI);
	...
	m_pUIOFct = pFlashUI->CreateEventSystem( "UIEntityTagsDynTex", IUIEventSystem::eEST_UI_TO_SYSTEM );
...
void CUIEntityDynTexTag::OnAddTaggedEntity( EntityId entityId, const char* uiElementName, const char* entityClass, const char* materialTemplate, const Vec3& offset, const char* idx)
{
	...
	if (pEntClass)
	{
		...
		IUIElement* pElement = pFlashUI->GetUIElement(uiElementName);
		if (pTagEntity && pElement)
		{
			...
			IComponentRender* pRenderProxy = (IComponentRender*) pTagEntity->GetComponent<IComponentRender>().get();
			if (pRenderProxy)
			{
				IRenderNode *pRenderNode = pRenderProxy->GetRenderNode();
				if (pRenderNode)
				{
#if NO_VIEWDIST_RATIO
					pRenderNode->SetViewDistanceMultiplier(256);
#endif
				}
			}

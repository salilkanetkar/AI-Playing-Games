...
void CUILobbyMP::InitEventSystem()
{
	...
	m_pUIFunctions = pFlashUI->CreateEventSystem("LobbyMP", IUIEventSystem::eEST_SYSTEM_TO_UI);
	...
	m_pUIEvents = pFlashUI->CreateEventSystem("LobbyMP", IUIEventSystem::eEST_UI_TO_SYSTEM);
	...
	pFlashUI->RegisterModule(this, "CUILobbyMP");
}
...
#ifdef CUT_LOBBY_OUT
	if(FakeGetLobby() && FakeGetLobby()->GetVoice())
	{
		FakeGetLobby()->GetVoice()->MuteLocalPlayer(0, mute);
	}
#endif
}
...
{
	//Retrieve friendlist
	ICryFriends* pFriends = FakeGetLobby()->GetFriends();
	unsigned int user = g_pGame->GetExclusiveControllerDeviceIndex();
	if(pFriends)
	{

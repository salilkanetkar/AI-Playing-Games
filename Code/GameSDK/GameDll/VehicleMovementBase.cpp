...
bool CVehicleMovementBase::Init(IVehicle* pVehicle, const CVehicleParams& table)
{
	...
	m_pIComponentAudio = m_pEntity->GetOrCreateComponent<IComponentAudio>();
	assert(m_pIComponentAudio.get());
	...
	m_pIComponentAudio->SetAuxAudioProxyOffset(SATLWorldPosition(m_enginePos));
...
{  
	...
	m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleSpeed], m_speed);
...
void CVehicleMovementBase::StopEngine()
{
	...
	m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleSpeed], 0.0f);
	m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleRPM], 0.0f);
...
void CVehicleMovementBase::OnVehicleEvent(EVehicleEvent event, const SVehicleEventParams& params)
{
	...
	case eVE_PassengerExit:
		{
			...
			{
				m_pIComponentAudio->SetSwitchState(m_audioControlIDs[eSID_VehicleINOUT], nInOutStateID);
			}
		}
		break;
	case eVE_Damaged:
	case eVE_Destroyed:
		{
			m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleDamage], GetSoundDamage());
	...
	case eVE_Repair:
		{
			...
			{
				m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleDamage], fSoundDamage);
...
{
	assert(m_pIComponentAudio && eSID>=0 && eSID<eSID_Max);

	const TAudioControlID id = m_audioControlIDs[eSID];
	if (id != INVALID_AUDIO_CONTROL_ID)
	{
		m_pIComponentAudio->ExecuteTrigger(id, eLSM_None);
	}
}
...
void CVehicleMovementBase::StopTrigger(EVehicleMovementSound eSID)
{
	...
	if (id != INVALID_AUDIO_CONTROL_ID)
	{
		m_pIComponentAudio->StopTrigger(id);
	}
}
...
Vec3 CVehicleMovementBase::GetWindPos(Vec3& posRad, Vec3& posLin)
{
  ...
  if (Intersect::Ray_AABB(ray, bounds, intersect))
  {
    ...
    if (IsProfilingMovement())
    {
      IPersistentDebug* pDebug = g_pGame->GetIGameFramework()->GetIPersistentDebug();
...
void CVehicleMovementBase::ResetAudioParams()
{
	if (m_pIComponentAudio)
	{
		m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleRPM], 0.0f);
		m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleSpeed], 0.0f);
		m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleDamage], 0.0f);
		m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleSlip], 0.0f);
		m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleStroke], 0.0f);
	}
}

...
void CVehiclePhysicsHelicopter::ProcessAI(SVehiclePhysicsHelicopterProcessAIParams& params)
{
	...
	float accel = (float)fsel(desiredSpeed - params.aiCurrentSpeed, 1.0f, 5.0f);
	...
	float maxForwardSpeed = (float)fsel(forwardSpeed, m_handling.maxSpeedForward, m_handling.maxSpeedBackward);

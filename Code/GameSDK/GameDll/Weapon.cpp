...
DECLARE_DEFAULT_COMPONENT_FACTORY(CWeapon, CWeapon)
...
void CWeapon::Update( SEntityUpdateContext& ctx, int update)
{
	...
	if (!IsDestroyed())
	{
		...
		m_delayedFireActionTimeOut = (float)fsel(-sprintFactor, max(m_delayedFireActionTimeOut-ctx.fFrameTime, 0.0f), m_delayedFireActionTimeOut);
...
//------------------------------------------------------------------------
void CWeapon::ReloadWeapon(bool force)
{
	CActor *pOwner = GetOwnerActor();

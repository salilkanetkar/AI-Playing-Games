				...
				if (distSquared >= sqr(maxNormalDistance))
				{
					...
					if (pPhysics && pPhysics->GetParams(&dimensions))
					{
						if (dimensions.bUseCapsule)
						{
							...
							float distance = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(physPrimitive.type, &physPrimitive, -v3PlayerToTarget, ent_all, NULL, 0, geom_colltype0 | geom_colltype_player, NULL, PhysicsForeignData(), 0, pSkipEntities, numSkipEntities);
							if (distance == 0.f)
							{
								return CE_YES;
							}

...
void CIronSight::ClearDoF()
{
	if (m_pWeapon->IsOwnerClient() && !m_pWeapon->IsModifying())
	{
		...
		gEnv->p3DEngine->SetPostEffectParam("Dof_FocusMinZ", m_zoomParams->zoomParams.dof_shoulderMinZ);
		gEnv->p3DEngine->SetPostEffectParam("Dof_FocusMinZScale", m_zoomParams->zoomParams.dof_shoulderMinZScale);
		gEnv->p3DEngine->SetPostEffectParam("FilterMaskedBlurring_Amount", 0.0f);
	}
}

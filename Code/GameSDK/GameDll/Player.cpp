		...
		if (cameraJnt >= 0)
		{
			...
			if (!IsThirdPerson())
			{
				...
				if (fadeoutNearWallThreshold > FLT_EPSILON)
				{
					...
					float hitDistance = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(physPrimitive.type, &physPrimitive, sweepDirection, entTypes, &pContact, 0, rwiFlags, 0, PhysicsForeignData(), 0, pSkipEntities, numSkipEntities);
					...
					else
					{
						...
						hitDistance = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(physPrimitive.type, &physPrimitive, ZERO, entTypes, &pContact, 0, rwiFlags, 0, PhysicsForeignData(), 0, pSkipEntities, numSkipEntities);
...
DECLARE_DEFAULT_COMPONENT_FACTORY(CPlayer,CPlayer)
...
, m_fOxygenLevel(1.0f)
{
	...
	m_pIComponentAudio = IComponentAudioPtr();
...
bool CPlayer::Init(IGameObject * pGameObject)
{
	...
	if(IComponentRender* pProxy = (IComponentRender*)pEntity->GetComponent<IComponentRender>().get())
	{
...
void CPlayer::InitLocalPlayer()
{
	...
	if (!(gEnv->bServer && g_pGame->IsGameSessionHostMigrating()))
	{
		...
		m_pIComponentAudio = GetEntity()->GetOrCreateComponent<IComponentAudio>();
		//m_pIComponentAudio->SetFlags(m_pIComponentAudio->GetFlags()|IComponentAudio::FLAG_DELEGATE_SOUND_ANIM_EVENTS);

		if (m_pIComponentAudio != NULL)
		{
			...
			if (nObjectSpeedSwitchID != INVALID_AUDIO_CONTROL_ID)
			{
				...
				if(nObjectSpeedTrackingOnStateID != INVALID_AUDIO_SWITCH_STATE_ID)
				{
					...
					m_pIComponentAudio->SetSwitchState(nObjectSpeedSwitchID, nObjectSpeedTrackingOnStateID);
				}
			}
		}
...
void CPlayer::SetChannelId (ChannelId id)
{
...
void CPlayer::Update(SEntityUpdateContext& ctx, int updateSlot)
{
...
#if HACK_OUT_NET_FEATURE
	// TODO: This code is trying to get the latency a player is seeing. GridMate doesn't know how to do this right now?!
	...
	if( gEnv->bMultiplayer )
	{
		...
		if( m_logPingTimer > LOG_PING_FREQUENCY )
		{
			...
			if( pGameLobby && pGameLobby->GetState() == eLS_Game )
			{
				...
				//FakeGetLobby()->GetMatchMaking()->GetSessionPlayerPing( conUID, &ping );
	...
	m_stats.zeroVelocityForTime = (float)fsel(m_stats.zeroVelocityForTime, m_stats.zeroVelocityForTime, 0.f) - ctx.fFrameTime;
...
void CPlayer::PrePhysicsUpdate()
{
	...
	if (m_pMovementController)
	{
		...
		const float frameTimeInv = (float)fsel(-frameTime, 0.0f, fres(frameTime + FLT_EPSILON));
...
float CPlayer::CalculatePseudoSpeed(bool wantSprint, float speedOverride) const
{
	...
	else
	{
		...
		float speed = (float) fsel(speedOverride, speedOverride, GetActorPhysics().velocity.len());

		const float ret = min((speed - MIN_WALK_SPEED) * fINV_MAGNITUDE, 1.0f);

		return (float)fsel(speed - MIN_WALK_SPEED, MIN_PSEUDO_SPEED + (ret * (MAX_PSEUDO_SPEED - MIN_PSEUDO_SPEED)), 0.0f);
		...
		if(IPhysicalEntity* pOriginalPhysEnt = pEntity->GetPhysics())
		{
			if(pOriginalPhysEnt->GetType() != PE_ARTICULATED)
			{
				...
				if(IComponentRender* pRenderProxy = (IComponentRender*)pEntity->GetComponent<IComponentRender>().get())
				{
					...
					{
						pRenderNode->SetViewDistanceMultiplier(g_pGameCVars->g_actorViewDistRatio);
					}
				}
			}
		}
...
void CPlayer::PostPhysicalize()
{
  ...
  if (!pPhysicalEnt)
	...
	if (pPhysEnt)
	{
		...
		if (isRemoteClient)
		{
			m_inertia = (float)fsel(g_pGameCVars->pl_clientInertia, g_pGameCVars->pl_clientInertia, m_inertia);
...
float CPlayer::GetRegenerationAmount(float frameTime)
{
	...
	if (!m_isHeadUnderWater)
	{
		...
		regenAmount = (float)fsel((timeThreshold-elapsedTime), 0.0f, recharge);
	}
...
void CPlayer::ExecuteFootStep(ICharacterInstance* pCharacter, const float frameTime, const int32 nFootJointID)
{
	...
	if (GetBoneID(boneId) >= 0)
	{
		...
		params.audioComponentEntityId = GetEntityId();
		//params.audioComponentEntityId = g_pGameCVars->g_FootstepSoundsFollowEntity ? GetEntityId() : 0;
		...
		params.audioComponentOffset= GetEntity()->GetWorldTM().GetInverted().TransformVector(params.pos - GetEntity()->GetWorldPos());
...
void CPlayer::ExecuteFoleySignal(ICharacterInstance* pCharacter, const float frameTime, const char* sFoleyActionName, const int32 nBoneJointID)
{
	...
	if (frameTime > 0.0f)
	{
		float const fInvFrameTime = fres(frameTime);
...
void CPlayer::NotifyObjectGrabbed(bool bIsGrab, EntityId objectId, bool bIsNPC, bool bIsTwoHanded /*= false*/, float fThrowSpeed)
{
	...
	IComponentScript* pScriptProx = (IComponentScript*)pEntity->GetComponent<IComponentScript>().get();
...
void CPlayer::AnimationEvent(ICharacterInstance *pCharacter, const AnimEventInstance &event)
{
	...
	if (isMainCharacter)
	{
		...
		else if (event.m_EventNameLowercaseCRC32 == audioTriggerCRC)
		{
			...
			if (isClient && m_pIComponentAudio)
			{
				...
				if (event.m_BonePathName && event.m_BonePathName[0] && pCharacter)
				{
					...
					if (nJointID >= 0)
					{
						...
						{
							nAudioProxyID = m_pIComponentAudio->CreateAuxAudioProxy();
							m_cJointAudioProxies[nJointID] = nAudioProxyID;
						}

						QuatT boneQuat(pSkeletonPose->GetAbsJointByID(nJointID));
						m_pIComponentAudio->SetAuxAudioProxyOffset(SATLWorldPosition(Matrix34(boneQuat)), nAudioProxyID);
					}
				}
				...
				{
					m_pIComponentAudio->ExecuteTrigger(nTriggerID, eLSM_None, nAudioProxyID);
				}
				...
				if(m_pIComponentAudio)
					m_pIComponentAudio->PlaySound(event.m_CustomParameter, offset, FORWARD_DIRECTION, flags, 0, eSoundSemantic_Animation, 0, 0);*/
			}
...
void CPlayer::UpdateFlashbangEffect(float frameTime)
{
	if (m_stats.flashBangStunTimer > 0.f)
	{
		...
		else
		{
			...
			float stunFracRemaining = clamp_tpl(m_stats.flashBangStunTimer * fres(m_stats.flashBangStunLength), 0.0f, 1.0f);
			...
			{
				blindFrac = clamp_tpl((stunFracRemaining * fres(stunFracBlindThresh)), 0.f, 1.f);
			}
...
const Vec3 CPlayer::GetFPCameraOffset() const
{
	...
	if (canOffset)
	{
		...
		case STANCE_SWIM:
			...
			break;
		case STANCE_CROUCH:
			frameOffset.z -= (float)fsel(lookDown, lookDown*zOffset, lookDown*zCrouchLookDownOffset);
			break;
		}
	}
	...
	if(!IsDead() && !IsClient())
	{
		...
		{		
			...
			if(!pTargetActor->IsDead())
			{
				...
				/*g_pGame->GetIGameFramework()->GetIPersistentDebug()->Begin("STEALTHKILL_NETPREKILL", false);
				g_pGame->GetIGameFramework()->GetIPersistentDebug()->AddSphere(pTargetEntity->GetWorldPos(), 0.2f, ColorF(1.f, 1.f, 0.f, 1.f), 120.f);
				g_pGame->GetIGameFramework()->GetIPersistentDebug()->AddLine(pTargetEntity->GetWorldPos(), pTargetEntity->GetWorldTM().GetColumn1() + pTargetEntity->GetWorldPos(), ColorF(1.f, 1.f, 0.f, 1.f), 120.f);
				g_pGame->GetIGameFramework()->GetIPersistentDebug()->AddSphere(pPlayerEntity->GetWorldPos(), 0.2f, ColorF(0.f, 1.f, 0.f, 1.f), 120.f);
				g_pGame->GetIGameFramework()->GetIPersistentDebug()->AddLine(pPlayerEntity->GetWorldPos(), pPlayerEntity->GetWorldTM().GetColumn1() + pPlayerEntity->GetWorldPos(), ColorF(0.f, 1.f, 0.f, 1.f), 120.f);*/
...
void CPlayer::CommitKnockDown()
{
	...
	IComponentPhysics *pPhysicsProxy=static_cast<IComponentPhysics *>(GetEntity()->GetComponent<IComponentPhysics>().get());
	if (pPhysicsProxy)
	{
		AABB bbox;
...
void CPlayer::RefillAmmo()
{
	if(gEnv->bMultiplayer)
	{
		...
		if(pCurrentWeapon && pCurrentWeapon->OutOfAmmo(false))
		{
			...
			pCurrentWeapon->ReloadWeapon(true);
			return;
		}
	}
...
//------------------------------------------------------------------------
void CPlayer::SerializeSpawnInfo(TSerialize ser , IEntity *)
{

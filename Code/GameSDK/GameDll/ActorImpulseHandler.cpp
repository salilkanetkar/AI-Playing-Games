...
{
	IComponentPhysics *pPhysicsProxy = static_cast<IComponentPhysics*>(m_actor.GetEntity()->GetComponent<IComponentPhysics>().get());
...
void CActorImpulseHandler::ApplyDeathImpulse(const HitInfo& lastHit)
{
	...
	if (itCandidate != m_pParams->impulseSets.end())
	{
		...
		for (SImpulseSet::ImpulseContainer::const_iterator it = impulseSet.impulses.begin(); it != itEnd; ++it)
		{
			...
			myImpulse = static_cast<float>(fsel(m_pParams->fMaxRagdollImpulse, min(m_pParams->fMaxRagdollImpulse, myImpulse), myImpulse));
...
void CActorImpulseHandler::ApplyDeathAngularImpulse(float fAngularImpulseScale, const Vec3& hitpos, const Vec3& playerPos, const HitInfo& lastHit)
{
	...
	angImpulse = min(gf_PI * 0.5f * m_pParams->fMass, fabs_tpl(angImpulse)) * static_cast<float>(fsel(angImpulse, 1.0f, -1.0f));
	angImpulse = static_cast<float>(fsel(fwdVec.dot(delta), -angImpulse, angImpulse));

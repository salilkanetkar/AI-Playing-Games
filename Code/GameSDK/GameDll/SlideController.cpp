...
public:
	...
	EStatus Update(float timePassed)
	{
		...
		else
		{
			...
			const float newLeanFactor	= (float)fsel(delta, min(m_leanFactor + step, targetLeanFactor), max(m_leanFactor - step, targetLeanFactor));
...
bool CSlideController::UpdateMovementRequest( CPlayer& player, float frameTime, const SActorFrameMovementParams& frameMovementParams, bool isNetSliding, bool isNetExitingSlide, SCharacterMoveRequest& movementRequest )
{
	...
	if(groundNormal.IsUnit())
	{
		...
		{ 
			...
			downHillAccel = (float)fsel(-maxDiff, slideCvars.max_downhill_acceleration, clamp_tpl(currentDiff/maxDiff, 0.0f, 1.0f) * slideCvars.max_downhill_acceleration);
		}
	}
	...
	const float gameSpeed = (float)fsel(-frameTime, 0.0f, (distanceTravelledLastFrame / (frameTime + FLT_EPSILON)));

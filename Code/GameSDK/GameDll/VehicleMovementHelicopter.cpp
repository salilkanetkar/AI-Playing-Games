...
void CVehicleMovementHelicopter::ProcessAI(const float deltaTime)
{
  ...
  float posNeg = (float)fsel(desiredSpeed - m_CurrentSpeed, 1.0f, -5.0f);
  ...
  else
  {
	...
	float rotSpeed		= (float)fsel(dirDiff.z, m_yawPerRoll, -m_yawPerRoll);
	...
	m_actionYaw	*= (float)fsel(absDiff - temp, 1.0f, multiplier);
  ...
  else
  {
	...
	rollSpeed *= deltaTime;
	rollSpeed		= (float)fsel(absDiff - rollSpeed, rollSpeed, absDiff);
	float roll		=(float) fsel(dirDiff.z, -rollSpeed, rollSpeed);

	float speedPerUnit	   = 1.5f;
	float desiredRollSpeed = absDiff * speedPerUnit * (float)fsel(dirDiff.z, 1.0f, -1.0f);
...
void CVehicleMovementHelicopter::CalculatePitch(const Ang3& worldAngles, const Vec3& desiredMoveDir, float currentSpeed2d, float desiredSpeed, float deltaTime)
{
	...
	float desiredPitch = -static_cast<float>(fsel(currentSpeed2d - desiredSpeed * 0.50f, currentSpeed2d, currentSpeed2d * kSpeedToPitchFactor)) / m_maxSpeed * gf_PI * 0.25f;

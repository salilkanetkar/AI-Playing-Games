...
// Description : Dual character proxy, to play animations on fp character and shadow character
...
bool CAnimationProxyDualCharacterBase::StartAnimation(IEntity *entity, const char* szAnimName, const CryCharAnimationParams& Params, float speedMultiplier)
{
	...
	if (pICharacter)
	{
		...
		if (g_pGameCVars->g_animatorDebug)
		{
			...
			if (animID < 0) 
			{
				g_pGame->GetIGameFramework()->GetIPersistentDebug()->Add2DText(string().Format("Missing %s", szAnimName).c_str(), 1.0f, col, 10.0f);
			}
		}

...
static void GetTalosInput(CPlayerRotation * pPlayerRotation, const CPlayer& rPlayer, float& x, float& z, const Vec3 playerView[4], float fFrameTime)
{
	...
	if(follow_target_id != 0)
	{
		...
		const float fHorzFinalAngle = (float)fsel(directionDotHorz, -angle, angle);
		const float fVertFinalAngle = (float)fsel(directionDotVert, angleToTargetV, -angleToTargetV);
...
void CPlayerRotation::ProcessNormalRoll( float frameTime )
{
	...
	if ((speed2 > 0.01f) && m_player.m_stats.inAir)
	{
		...
		const float maxSpeedInverse = (float)fsel(-maxSpeed, 1.0f, fres(maxSpeed + FLT_EPSILON));
...
void CPlayerRotation::ProcessAngularImpulses( float frameTime )
{
	...
	{
		m_angularImpulse *= min(m_angularImpulseTime * fres(m_angularImpulseTimeMax), 1.0f);
	...
	//Further limit the view if necessary
	{
		...
		if ((limitH+limitV+fabsf(limitVUp)+fabsf(limitVDown)) > 0.0f)
		{
			...
			if (limitV)
			{
				...
				m_deltaAngles.x = m_deltaAngles.x + (float)fsel(deltaX, 0.0f, deltaX * (float)fsel(limit.x, 1.0f, -1.0f));
			}
			...
			if (limitVUp || limitVDown)
			{
				...
				const float fDeltaXUpIncrement = (float)fsel( deltaXUp, 0.0f, deltaXUp);
				fNewDeltaX = fNewDeltaX + (float)fsel(-fabsf(limitVUp), 0.0f, fDeltaXUpIncrement);

				const float deltaXDown(limitVDown - limit.x);

				const float fDeltaXDownIncrement = (float)fsel( deltaXDown, deltaXDown, 0.0f);
				fNewDeltaX = fNewDeltaX + (float)fsel(-fabsf(limitVDown), 0.0f, fDeltaXDownIncrement);
			...
			if (limitH)
			{
				...
				m_deltaAngles.z = m_deltaAngles.z + (float)fsel(deltaZ, 0.0f, deltaZ * (float)fsel(limit.z, 1.0f, -1.0f));
			}
		}
	}
}
...
void CPlayerRotation::TargetAimAssistance(CWeapon* pWeapon, float& followH, float& followV, float& scale, float& _fZoomAmount, const Vec3 playerView[4])
{
	...
	if(pWeapon)
	{
		...
		fMaxDistMult                                = (fMaxAssistDist + fMaxAssistDistDiff) * fres(fMaxAssistDist);
	}
	else
	{
		_fZoomAmount = 0.0f;
	...
	for (int i = 0; i < targetCount; ++i)
	{
		...
		const float worst_follow_fraction = (float)fsel(angle_follow_fraction - angle_follow_fractionV, angle_follow_fraction, angle_follow_fractionV);
		...
		fBestTargetDistance = (float)fsel(fCombinedModifier - bestScale, fBestTargetDistance, distance);
		...
		if (follow_vote > follow_vote_leader)
		{
			...
			if ( m_follow_target_id == target.entityId)
			{
				...
				followH = follow_fraction * (float)fsel(angleDot, angleH, -angleH);
				followV = follow_fraction * (float)fsel(angleDotV, angleV, -angleV);	
		...
		else if (!follow_target_id && (radius_snap > 0.0f))
		{
			...
			{
				float t = 0.0f;
				const float snap_fraction = 1.0f - (Distance::Point_Lineseg(targetPos, lineSegment, t) * (float)fres(radius_snap));
...
void CPlayerRotation::ProcessTargetAssistance( IItem* pCurrentPlayerItem )
{
	...
	float fFollowAssistScale = (absInput * (1.0f - fZoomAmount)) + min(absInput * fZoomAmount * fres(g_pGameCVars->aim_assistInputForFullFollow_Ironsight), 1.0f);
...
void CPlayerRotation::ProcessNormal( float frameTime )
{
	...
	newViewAngles.z = (float)fsel(  newViewAngles.z - (2.0f * gf_PI2),  newViewAngles.z - (4.0f * gf_PI2), newViewAngles.z);
	newViewAngles.z = (float)fsel(-(newViewAngles.z + (2.0f * gf_PI2)), newViewAngles.z + (4.0f * gf_PI2), newViewAngles.z);
...
{
	const float leanAmt = (float)fsel(0.01f - fabsf(movement.desiredLean), 0.0f, movement.desiredLean);
	...
	else
	{
		...
		if (validHit)
		{
			...
			const float invDist = dist>0.f ? fres(dist) : 0.f;
...
void CPlayerRotation::ProcessForcedLookDirection( const Quat& lastViewQuat, float frameTime )
{
	...
	forceLook *= (float)fres(forceLookLen);

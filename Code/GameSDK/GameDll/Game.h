...
public:
    ...
    virtual EGameSDKPlatform GetPlatform() const;
  ...
  // ~IGameFrameworkListener
	...
	GameSDK::CScriptBind_LightningArc* GetLightningArcScriptBind() { return m_pScriptBindLightningArc; }
    ...
    ILINE GameSDK::CLightningGameEffect* GetLightningGameEffect() { return m_pLightningGameEffect; }
  ...
  CDLCManager*		GetDLCManager()			{ return m_pDLCManager; }
    std::shared_ptr<IPrefabManager> GetPrefabManager() { return m_pPrefabManager; }
...
protected:
	...
	{
        EGameSDKPlatform	platformId;
		BYTE		devices;				// Devices to use when registering actions

        SPlatformInfo(EGameSDKPlatform _platformId = ePlatform_Unknown) : platformId(_platformId), devices(eAID_KeyboardMouse | eAID_XboxPad | eAID_PS4Pad) {}
	};
	SPlatformInfo m_platformInfo;

	void ReleaseActionMaps();
	bool ReadProfile(XmlNodeRef &rootNode);
    bool ReadProfilePlatform(XmlNodeRef &platformsNode, EGameSDKPlatform platformId);
  ...
  static void CmdVote(IConsoleCmdArgs* pArgs);
	...
	GameSDK::CScriptBind_Boids* m_pScriptBindBoids;
	...
	GameSDK::CScriptBind_LightningArc* m_pScriptBindLightningArc;
    ...
    GameSDK::CLightningGameEffect  *m_pLightningGameEffect;
	CParameterGameEffect *m_pParameterGameEffect;

	std::shared_ptr<IPrefabManager>				m_pPrefabManager;

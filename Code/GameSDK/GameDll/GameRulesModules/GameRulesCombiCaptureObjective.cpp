...
// Description : Implementation of a  combination capture  objective
...
void CGameRulesCombiCaptureObjective::SCaptureEntity::SetEnabled(const bool enable, const bool updateIcon, const SHoldEntityDetails* pDetails)
{
	...
	if (pDetails && (pDetails->m_pAdditionalData == this))
	{
		if (enable != m_enabled)
		{
			...
			{
				if (IComponentRender* renderProxy=static_cast<IComponentRender*>(pEntity->GetComponent<IComponentRender>().get()))
				{
...
//------------------------------------------------------------------------
void CGameRulesCombiCaptureObjective::OnClientConnect( ChannelId channelId, bool isReset, EntityId playerId )
{

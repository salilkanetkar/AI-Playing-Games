...
// Description : Game rules module to handle player scores and stats
	...
	if (pGameRules)
	{
		...
		if (currTime - m_lastUpdatedPings > CGameRulesStandardPlayerStats::GR_PLAYER_STATS_PING_UPDATE_INTERVAL)
		{
			...
			for (TPlayerStats::iterator it=m_playerStats.begin(); it!=m_playerStats.end(); ++it)
			{
				...
				if (actor)
				{
#if HACK_OUT_NET_LATENCY
					// NO way to measure this currently.
...
//	INetContext*  pNetContext = g_pGame->GetIGameFramework()->GetNetContext();
	...
	for (TPlayerStats::iterator it=m_playerStats.begin(); it!=m_playerStats.end(); ++it)
	{
		...
		{
			gEnv->pNetwork->ChangedAspects(it->playerId, CPlayer::ASPECT_PLAYERSTATS_SERVER);
		}
	}
}
	...
	if (targetActor)
	{
		...
		if (playerStat)
		{
			...
			if (gEnv->bServer)
			{
				...
				if (gEnv && gEnv->pNetwork)
				{
					gEnv->pNetwork->ChangedAspects(info.targetId, CPlayer::ASPECT_PLAYERSTATS_SERVER);
				}
			}
		}
	}
	...
	if (shooterActor && (shooterActor != targetActor))
	{
		...
		if (playerStats)
		{
			...
			if (gEnv->bServer)
			{
				...
				if (gEnv && gEnv->pNetwork)
				{
					gEnv->pNetwork->ChangedAspects(info.shooterId, CPlayer::ASPECT_PLAYERSTATS_SERVER);
				}
			}
		}
	}
...
//			INetContext *pNetContext = g_pGame->GetIGameFramework()->GetNetContext();
			if (gEnv && gEnv->pNetwork)
			{
				gEnv->pNetwork->ChangedAspects(playerId, CPlayer::ASPECT_PLAYERSTATS_SERVER);
			}
		}
...
void CGameRulesStandardPlayerStats::EntityRevived(EntityId entityId)
{
	if (gEnv->bServer)
	{
		if (SGameRulesPlayerStat* s=GetPlayerStatsInternal(entityId))
		{
			...
			if (gEnv && gEnv->pNetwork)
			{
				gEnv->pNetwork->ChangedAspects(entityId, CPlayer::ASPECT_PLAYERSTATS_SERVER);
			}
		}
	}
}
...
//	INetContext*  pNetContext = g_pGame->GetIGameFramework()->GetNetContext();
	...
	for (unsigned int i=0; i<numPlayers; i++)
	{
		...
		if (gEnv->bServer)
		{
			if (gEnv && gEnv->pNetwork)
			{
				gEnv->pNetwork->ChangedAspects(pStats->playerId, CPlayer::ASPECT_PLAYERSTATS_SERVER);
			}
		}
	}
}

...
void CGameRulesMPSpectator::ChangeSpectatorMode(const IActor *pActor, uint8 mode, EntityId targetId, bool resetAll, bool force)
{
	...
	else if (pActor->IsClient())
	{
		if(m_ClientInfo.m_mode != mode || m_ClientInfo.m_targetId != targetId || m_ClientInfo.m_fRequestTimer < 0.0f)
		{
			...
			if(ICryLobby * pLobby = FakeGetLobby())
			{
				CryPing ping = 0;
...
// -----------------------------------------------------------------------
bool CGameRulesMPSpectator::GetModeFromChannelSpectatorMap(ChannelId channelId, int* outMode) const
{
	bool  found = false;
...
// -----------------------------------------------------------------------
void CGameRulesMPSpectator::FindAndRemoveFromChannelSpectatorMap(ChannelId channelId)
{
...
void CGameRulesMPSpectator::GetPostDeathDisplayDurations( EntityId playerId, int teamWhenKilled, bool isSuicide, bool isBulletTimeKill, float& rDeathCam, float& rKillerCam, float& rKillCam ) const
{
	...
	{
		// Just show DeathCam.
		rDeathCam = (float)fsel( reviveTime - g_pGameCVars->g_tpdeathcam_timeOutSuicide, g_pGameCVars->g_tpdeathcam_timeOutSuicide, reviveTime );
		rKillerCam = 0.f;
		rKillCam = 0.f;
	}
	else
	{
		...
		const float remainingTime = (float)fsel((reviveTime-killCamTime)-killCamViableUseTime, reviveTime-killCamTime, killCamViableUseTime );
		...
		else if( remainingTime >= shortestTimeForBoth && idealTotalTime>FLT_EPSILON )
		{
			...
			const float invTotalSplitAmount = fres(deathSplitAmount+killerSplitAmount);

...
// Description : Objective handler for extraction gamemode
...
void CGameRulesObjective_Extraction::PlayerDropsPickupCommon(EntityId playerId, SPickup *pickup, bool newDrop)
{
	...
	if (gEnv->IsClient())
	{
		...
		float dst = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(sphere.type, &sphere, end-sphere.center, PROBE_TYPES,
			&pContact, 0, PROBE_FLAGS, 0, PhysicsForeignData(), 0);
...
void CGameRulesObjective_Extraction::OnClientDisconnect(ChannelId channelId, EntityId playerId)
{
	if (gEnv->bServer)
	{
...
void CGameRulesObjective_Extraction::OnClientEnteredGame(ChannelId channelId, bool isReset, EntityId playerId)
{

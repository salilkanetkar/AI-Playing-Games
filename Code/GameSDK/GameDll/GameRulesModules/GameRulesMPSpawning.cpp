	...
	if(pLocalActorImpl != NULL && pLocalActorImpl->IsDead() && (!stateModule || (stateModule->GetGameState() != IGameRulesStateModule::EGRS_PostGame)) && (spectatorState == CActor::eASS_Ingame || shouldRequestRevive))
	{
		if (!pObjectivesModule || pObjectivesModule->CanPlayerRevive(localActorEntityId))
		{
			...
			else
			{
				...
				m_fPrecacheTimer = (float)fsel(g_pGameCVars->g_spawnPrecacheTimeBeforeRevive-timeTillAutoRevive, 0.01f, timeTillAutoRevive-g_pGameCVars->g_spawnPrecacheTimeBeforeRevive);
		...
		if(pActor)
		{
			...
			if (g_pGame->GetHostMigrationState() == CGame::eHMS_NotMigrating)
			{
				...
				if (it != m_playerValues.end())
				{
					...
					{
						// TODO: HACK_NET_OUT_PING_TIME: need to find out how to get ping time w/ GridMate
						//const INetChannel*  pNetChannel = gEnv->pGame->GetIGameFramework()->GetNetChannel(uActorChannel);
						const float  netLag = 0.0f; // ((pActor->IsClient() || !pNetChannel) ? 0 : ((pNetChannel->GetPing(true) * 1000.f) * 0.5f));  // GetPing() is the round journey, so 0.5 gives us the 1-way trip.
						...
						//if (pNetChannel)
						{
							// TODO: HACK_NET_OUT: figure out how to get Ping w/ GridMate
...
bool CGameRulesMPSpawningBase::TestSpawnLocationWithEnvironment(const IEntity *pSpawn, EntityId playerId, float offset, float height) const
{
	...
	float dst = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(sphere.type, &sphere, end-sphere.center, ent_static|ent_terrain|ent_rigid|ent_sleeping_rigid|ent_living,
		&pContact, 0, (geom_colltype_player << rwi_colltype_bit) | rwi_stop_at_pierceable, 0, PhysicsForeignData(), 0, &pPlayerPhysics, pPlayerPhysics ? 1 : 0);

...
// Description : Implementation of power struggle objectives
...
float CGameRulesObjective_PowerStruggle::SmoothlyUpdateValue(float currentVal, float desiredVal, float updateRate, float frameTime)
{
	...
	float fMax = (float)fsel( fDiff, desiredVal, FLT_MAX);
	float fMin = (float)fsel(-fDiff, desiredVal, FLT_MIN);
...
//------------------------------------------------------------------------
void CGameRulesObjective_PowerStruggle::PostInitClient (ChannelId channelId)
{
	CRY_ASSERT(gEnv->bServer);
...
//------------------------------------------------------------------------
void CGameRulesObjective_PowerStruggle::OnClientDisconnect(ChannelId channelId, EntityId playerId)
{
...
void CGameRulesObjective_PowerStruggle::Client_ShakeCameraForSpear(const SNodeInfo *pNodeInfo)
{
	...
	if (pView)
	{
		...
		const float shakeFactor = min(1.0f, deltaDistanceFromShake * fres(shakeRadiusRange));

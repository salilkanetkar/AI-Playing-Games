...
// Description : Base implementation for a take and hold objective
...
//------------------------------------------------------------------------
void CGameRulesHoldObjectiveBase::OnClientDisconnect( ChannelId channelId, EntityId playerId )
{
	for (int i = 0; i < HOLD_OBJECTIVE_MAX_ENTITIES; ++ i)
	{
		SHoldEntityDetails *pDetails = &m_entities[i];
...
void CGameRulesHoldObjectiveBase::UpdateEffect(float frameTime)
{
	...
	if(pRingEntity)
	{
		...
		// Set ring color
		IComponentRender* pRenderProxy = (IComponentRender*)pRingEntity->GetComponent<IComponentRender>().get();
		if(pRenderProxy)
		{

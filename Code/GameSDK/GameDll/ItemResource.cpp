...
/*			if (slot == eIGS_FirstPerson)
	...
	else if (useParentMaterial && pParentEntity)
	{
		...
		IComponentRender* pParentRenderProxy = static_cast<IComponentRender*>(pParentEntity->GetComponent<IComponentRender>().get());
		if (pParentCharacter)
...
bool CItem::PlayFragment(IAction* pAction, float speedOverride, float timeOverride, float animWeight, float ffeedbackWeight, bool concentratedFire)
{
	...
	float speed = (float)fsel(-speedOverride, 1.0f, speedOverride);
...
//	IComponentAudio *pIComponentAudio = GetAudioProxy(false);
//	if (pIComponentAudio)
//	{
//		if(synchSound)
//			pIComponentAudio->StopSound(id, ESoundStopMode_OnSyncPoint);
//		else
//			pIComponentAudio->StopSound(id);
//	}
//}
...
void CItem::Quiet()
{
	...
	if (pIComponentAudio)
	{
		pIComponentAudio->StopAllSounds();
	}*/
}
...
//	IComponentAudio *pIComponentAudio = GetAudioProxy(false);
//	if (pIComponentAudio)
//		return pIComponentAudio->GetSound(id);
//
//	return 0;
//}

//------------------------------------------------------------------------
IComponentAudio *CItem::GetAudioProxy(bool create)
{
	IComponentAudio *pIComponentAudio = (IComponentAudio *)GetEntity()->GetComponent<IComponentAudio>().get();

	if (!pIComponentAudio && create)
		pIComponentAudio = static_cast<IComponentAudio *> (GetEntity()->GetOrCreateComponent<IComponentAudio>().get());

	return pIComponentAudio;
}

//------------------------------------------------------------------------
IComponentRender *CItem::GetRenderProxy(bool create)
{
	IComponentRender *pRenderProxy = (IComponentRender *)GetEntity()->GetComponent<IComponentRender>().get();
	if (!pRenderProxy && create)
		pRenderProxy = static_cast<IComponentRender *> (GetEntity()->GetOrCreateComponent<IComponentRender>().get());

	return pRenderProxy;
}

//------------------------------------------------------------------------
IComponentPhysics *CItem::GetPhysicalProxy(bool create)
{
	IComponentPhysics *pPhysicalProxy = (IComponentPhysics *)GetEntity()->GetComponent<IComponentPhysics>().get();
	if (!pPhysicalProxy && create)
		pPhysicalProxy = static_cast<IComponentPhysics *> (GetEntity()->GetOrCreateComponent<IComponentPhysics>().get());

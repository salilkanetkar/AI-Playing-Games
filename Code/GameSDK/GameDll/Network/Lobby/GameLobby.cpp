...
CGameLobby::CGameLobby( CGameLobbyManager* pMgr )
{
	...
	// TODO: HACK_NET_OUT: is this required any more with GridMate?
...
CGameLobby::~CGameLobby()
{
	...
	// TODO: HACK_NET_OUT: Is this required anymore with GridMate?	
...
void CGameLobby::Update(float dt)
{
	...
	if(s_pGameLobbyCVars->gl_voip_debug)
	{
		...
		for(int i = 0; i < nameSize; ++i)
		{
			...
			if (userId.IsValid())
			{
				ICryVoice *pCryVoice = 0;// FakeGetLobby()->GetVoice();
	...
	if(m_state == eLS_Lobby)
	{
		...
		{
			if (m_bHasUserList && (m_nameList.Size() == 1) && (m_server == false) && (FakeGetLobby()->GetMatchMaking()->GetSessionHostMigrationState(m_currentSession) == eHMS_Idle))
			{
...
void CGameLobby::MatchmakingEnsureBestHostCallback( CryLobbyTaskID taskID, ECryLobbyError error, void* arg )
{
	...
	if (s_pGameLobbyCVars->gl_debugForceLobbyMigrations)
	{
		//ICryMatchMaking *pMatchMaking = 0;// FakeGetLobby()->GetMatchMaking();
...
void CGameLobby::OnGameStarting()
{
	...
	CryUserID localUser = FakeGetLobby()->GetLobbyService()->GetUserID(g_pGame->GetExclusiveControllerDeviceIndex());
...
ECryLobbyError CGameLobby::DoUpdateLocalUserData(ICryMatchMaking *pMatchMaking, CryLobbyTaskID &taskId)
{
	...
	//ICryMatchMaking *pMatchmaking = FakeGetLobby()->GetLobbyService()->GetMatchMaking();
	//uint32 userIndex = g_pGame->GetExclusiveControllerDeviceIndex();
	ECryLobbyError result = 0;// pMatchmaking->SessionSetLocalUserData(m_currentSession, &taskId, userIndex, localUserData, sizeof(localUserData), MatchmakingLocalUserDataCallback, this);
...
void CGameLobby::MatchmakingSessionCreateCallback(CryLobbyTaskID taskID, ECryLobbyError error, CrySessionHandle h, void* arg)
{
	...
	if (pLobby->NetworkCallbackReceived(taskID, error))
	{
		ICryMatchMaking *pMatchmaking = 0;// FakeGetLobby()->GetLobbyService()->GetMatchMaking();
...
void CGameLobby::InsertUser(SCryUserInfoResult* user)
{
	...
	if (user->m_userID.IsValid())
	{
		...
		{
#if 0
			CryUserID localUser = 0;// FakeGetLobby()->GetLobbyService()->GetUserID(g_pGame->GetExclusiveControllerDeviceIndex());
	...
	if( CMatchmakingTelemetry* pMMTel = g_pGame->GetMatchMakingTelemetry() )
	{
		if( pMMTel->GetCurrentTranscriptType() == CMatchmakingTelemetry::eMMTelTranscript_QuickMatch )
		{
			...
			//CryUserID localUserId = FakeGetLobby()->GetLobbyService()->GetUserID( currentUserIndex );
...
void CGameLobby::RemoveUser(SCryUserInfoResult* user)
{
	...
	else if (m_state == eLS_Game)
	{
		...
		if (pGameRules)
		{
			ChannelId channelId = (int) user->m_conID.m_uid;
...
//------------------------------------------------------------------------
void CGameLobby::GetProgressionInfoByChannel(ChannelId channelId, uint8 &rank, uint8 &reincarnations)
{
	CrySessionHandle hdl = m_currentSession;

	if(hdl != CrySessionInvalidHandle)
	{
		SCryMatchMakingConnectionUID temp = FakeGetLobby()->GetMatchMaking()->GetConnectionUIDFromGameSessionHandleAndChannelID(hdl, channelId);
...
//------------------------------------------------------------------------
void CGameLobby::SetProgressionInfoByChannel(ChannelId channelId, uint8 rank, uint8 reincarnations)
{
	assert(m_server);
	...
	if(hdl != CrySessionInvalidHandle)
	{
		SCryMatchMakingConnectionUID connectionUid = FakeGetLobby()->GetMatchMaking()->GetConnectionUIDFromGameSessionHandleAndChannelID(hdl, channelId);
...
//------------------------------------------------------------------------
void CGameLobby::GetPlayerNameFromChannelId(ChannelId channelId, CryFixedStringT<CRYLOBBY_USER_NAME_LENGTH> &name)
{
	SCryMatchMakingConnectionUID temp = FakeGetLobby()->GetMatchMaking()->GetConnectionUIDFromGameSessionHandleAndChannelID(m_currentSession, channelId);
	SSessionNames::SSessionName *pPlayer = m_nameList.GetSessionName(temp, true);
...
void CGameLobby::GetClanTagFromChannelId(ChannelId channelId, CryFixedStringT<CLAN_TAG_LENGTH> &name)
{
	SCryMatchMakingConnectionUID temp = FakeGetLobby()->GetMatchMaking()->GetConnectionUIDFromGameSessionHandleAndChannelID(m_currentSession, channelId);
	SSessionNames::SSessionName *pPlayer = m_nameList.GetSessionName(temp, true);
	if (pPlayer)
...
//---------------------------------------
bool CGameLobby::IsLocalChannelUser(ChannelId channelId)
{
...
bool CGameLobby::GetPlayerDisplayNameFromEntity(EntityId entityId, CryFixedStringT<DISPLAY_NAME_LENGTH> &displayName)
{
	...
	const ChannelId channelId = pGameObject ? pGameObject->GetChannelId() : 0;
	if (channelId)
	{
...
//---------------------------------------
bool CGameLobby::GetPlayerDisplayNameFromChannelId(ChannelId channelId, CryFixedStringT<DISPLAY_NAME_LENGTH> &displayName)
{
...
//---------------------------------------
int CGameLobby::GetTeamByChannelId( ChannelId channelId )
{
	int teamId = 0;
...
//---------------------------------------
SCryMatchMakingConnectionUID CGameLobby::GetConnectionUIDFromChannelID(ChannelId channelId)
{
...
{
	if( ICryMatchMaking *pMatchMaking = FakeGetLobby()->GetMatchMaking() )
	{
		CryPing ping;
...
//---------------------------------------
CryUserID CGameLobby::GetUserIDFromChannelID(ChannelId channelId)
{
	CryUserID userId = CryUserInvalidID;

	if (gEnv->pNetwork && FakeGetLobby())
	{
		if (ICryMatchMaking* pMatchMaking = FakeGetLobby()->GetMatchMaking())
		{
...
//---------------------------------------
void CGameLobby::MutePlayerByChannelId( ChannelId channelId, bool mute, int reason )
{
...
void CGameLobby::MutePlayerBySessionName( SSessionNames::SSessionName *pUser, bool mute, int reason )
{
	...
	{
		ICryVoice *pCryVoice = FakeGetLobby()->GetVoice();
		uint32 userIndex = g_pGame->GetExclusiveControllerDeviceIndex();
		if (pCryVoice)
		{
...
//---------------------------------------
ELobbyVOIPState CGameLobby::GetVoiceState(ChannelId channelId)
{
...
IHostMigrationEventListener::EHostMigrationReturn CGameLobby::OnFinalise( SHostMigrationInfo& hostMigrationInfo, HMStateType& state )
{
	...
	if (isNewServer)
	{
		...
		ICryMatchMaking *pMatchMaking = 0; // gEnv->pLobby->GetMatchMaking();
...
void CGameLobby::SetLobbyService(ECryLobbyService lobbyService)
{
	...
	if (FakeGetLobby())
	{
	const bool allowJoinMultipleSessions = FakeGetLobby()->GetLobbyServiceFlag( lobbyService, eCLSF_AllowMultipleSessions );
	g_pGame->GetSquadManager()->Enable( allowJoinMultipleSessions, true );
	FakeGetLobby()->SetLobbyService(lobbyService);
}
}
...
void CGameLobby::TaskStartedCallback( CLobbyTaskQueue::ESessionTask task, void *pArg )
{
	...
	if (pGameLobby)
	{
		...
		ICryLobby *pCryLobby = FakeGetLobby();
		if (pCryLobby)
		{
...
//-------------------------------------------------------------------------
uint16 CGameLobby::GetSkillRanking( ChannelId channelId )
{
...
//-------------------------------------------------------------------------
SSessionNames::SSessionName* CGameLobby::GetSessionNameByChannelId(ChannelId channelId)
{
...
bool CGameLobby::GetSpectatorStatusFromChannelId( ChannelId channelId )
{
	CrySessionHandle mySession = m_currentSession;
...
void CGameLobby::UpdatePingKicker(float frameTime)
{
	...
	if (pingLimit)
	{
		...
		{
			ICryMatchMaking *pMatchMaking = FakeGetLobby()->GetMatchMaking();
...
//-------------------------------------------------------------------------
void CGameLobby::OnPlayerSpawned(ChannelId channelId)
{
...
//-------------------------------------------------------------------------
void CGameLobby::OnPlayerSwitchedTeam( ChannelId channelId, int teamId )
{
	SCryMatchMakingConnectionUID uid = GetConnectionUIDFromChannelID(channelId);
	if (uid != CryMatchMakingInvalidConnectionUID)
	{

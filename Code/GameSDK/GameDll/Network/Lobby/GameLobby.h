...
public:
	...
	void GetProgressionInfoByChannel(ChannelId channelId, uint8 &rank, uint8 &reincarnations);
	void SetProgressionInfoByChannel(ChannelId channelId, uint8 rank, uint8 reincarnations);

	void GetPlayerNameFromChannelId(ChannelId channelId, CryFixedStringT<CRYLOBBY_USER_NAME_LENGTH> &name);
	void GetClanTagFromChannelId(ChannelId channelId, CryFixedStringT<CLAN_TAG_LENGTH> &name);
	void LocalUserDataUpdated();

	bool IsLocalChannelUser(ChannelId channelId);
	...
	bool GetPlayerDisplayNameFromChannelId(ChannelId channelId, CryFixedStringT<DISPLAY_NAME_LENGTH> &displayName);

	bool GetSpectatorStatusFromChannelId(ChannelId channelId);
	...
	CryUserID GetUserIDFromChannelID(ChannelId channelId);
	int GetTeamByChannelId(ChannelId channelId);
	int GetTeamByConnectionUID( SCryMatchMakingConnectionUID uid );
	SCryMatchMakingConnectionUID GetConnectionUIDFromChannelID(ChannelId channelId);
	CryPing GetCurrentPingToHost();

	inline void OnSquadChanged() { m_squadDirty = true; }

	void MutePlayerByChannelId(ChannelId channelId, bool mute, int reason);
	int GetPlayerMuteReason(CryUserID userId);
	ELobbyVOIPState GetVoiceState(ChannelId channelId);
	...
	inline bool IsOnlineGame() { return false; /* (FakeGetLobby()->GetLobbyServiceType() == eCLS_Online); */ }
	...
	SSessionNames::SSessionName* GetSessionNameByChannelId(ChannelId channelId);
	...
	uint16 GetSkillRanking(ChannelId channelId);
	...
	void OnPlayerSpawned(ChannelId channelId);
	void OnPlayerSwitchedTeam(ChannelId channelId, int teamId);

	...
	if (i > removeThreshold)
	{
		...
		for(--i;i>=0;i--)
		{
			...
			if (pEntity)
			{
				...
				if (pEntity->GetComponent<IComponentFlowGraph>().get() != 0)
					continue;
				...
				// get bounding box
				if (IComponentPhysics* pPhysProxy = (IComponentPhysics*)pEntity->GetComponent<IComponentPhysics>().get())
				{
					AABB aabb;
...
IMPLEMENT_RMI(CGameRules, SvVote)
{
	...
	IActor* pActor = GetActorByChannelId(gEnv->pNetwork->GetServerChannelId());
	if(pActor)
		Vote(pActor, true);
	return true;
}
...
{
	CGameLobby * pGameLobby = 0; // g_pGame->GetGameLobby();
	IEntity * pEntity = gEnv->pEntitySystem->GetEntity(params.entityId);
...
IMPLEMENT_RMI(CGameRules, SvModuleRMIOnAction)
{
	...
	if (IActor* pActor= GetActorByChannelId(gEnv->pNetwork->GetServerChannelId()))  // sender client could've disconnected since sending RMI (possibly?)
	{
...
IMPLEMENT_RMI(CGameRules, SvSetEquipmentLoadout)
{
	...
	{
		pEquipmentLoadout->SvSetClientLoadout(gEnv->pNetwork->GetServerChannelId(), params);
	}
...
IMPLEMENT_RMI(CGameRules, ClPredictionFailed)
{
	...
	// TODO: Not sure what we should be asserting here instead of IsBound.
	//assert(pEntity && gEnv && gEnv->pNetwork && !gEnv->pNetwork->IsBound(params.predictionHandle));

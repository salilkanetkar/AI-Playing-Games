...
#define EQUIPMENT_PACKAGE_TYPES(f) \
	...
	void ClSendCurrentEquipmentLoadout(ChannelId channelId);
	
	void SvSetClientLoadout(ChannelId channelId, const CGameRules::EquipmentLoadoutParams &params);

	void SvAssignClientEquipmentLoadout(ChannelId channelId, int playerId);
	void ClAssignLastSentEquipmentLoadout(CActor *pActor);

	bool SvHasClientEquipmentLoadout(ChannelId channelId);
	void SvRemoveClientEquipmentLoadout(ChannelId channelId);

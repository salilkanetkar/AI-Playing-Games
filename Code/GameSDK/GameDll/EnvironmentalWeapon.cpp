...
DECLARE_DEFAULT_COMPONENT_FACTORY(CEnvironmentalWeapon, CEnvironmentalWeapon)
...
void CEnvironmentalWeapon::SetOwner(EntityId ownerId)
{
	...
	if(m_OwnerId != ownerId)
	{
		...
		if(IComponentPhysics* pPhysicsProxy = static_cast<IComponentPhysics*>(GetEntity()->GetComponent<IComponentPhysics>().get()))
		{
...
void CEnvironmentalWeapon::TrackWeaponVelocity(const float dt)
{
	if(m_pVelocityTracker)
	{
		...
		m_weaponTrackerVelocity *= fres(dt); 
	}
#ifndef _RELEASE
	else
	{
...
void CEnvironmentalWeapon::UpdateGlassBreakParams()
{
	if(m_logicFlags & ELogicFlag_damagePhaseActive)
	{
		...
		{
			float speedScale = fres(sqrt_tpl(speedSq)) * maxSpeed;
...
void CEnvironmentalWeapon::PerformGroundClearanceAdjust( const CActor* pOwner, const bool chargedThrown )
{
	...
	if(intersectionAssist.Enabled())
	{
		if(chargedThrown)
		{
			...
			float fFloorPenetrationAdjust = static_cast<float>(fsel( entLowestZVal-playerLowestZVal, 0.f, playerLowestZVal - entLowestZVal ));
			...
			if( IVehicleSystem* pVSystem = gEnv->pGame->GetIGameFramework()->GetIVehicleSystem() )
			{
				if( IVehicle* pVehicle = pVSystem->GetVehicle( m_parentVehicleId ) )
				{
					...
					if (gEnv && gEnv->pNetwork)
					{
						...
						SVehicleEventParams	vehicleEventParams;
						vehicleEventParams.entityId	= GetEntityId();
						vehicleEventParams.iParam = gEnv->pNetwork->LocalEntityIdToServerEntityId(vehicleEventParams.entityId);
...
void CEnvironmentalWeapon::InitClient (ChannelId channelId)
{
...
{
	CGameRules* pGameRules = g_pGame->GetGameRules();
	IActor* pActor = 0; // pGameRules ? pGameRules->GetActorByChannelId(g_pGame->GetIGameFramework()->GetGameChannelId(pNetChannel)) : NULL;
	if(pActor)
	{
...
//------------------------------------------------------------------------
void CEnvironmentalWeapon::SerializeSpawnInfo( TSerialize ser, IEntity * )
{
	...
	if (pTable != NULL)
	{
		...
		if (pTable->GetValue("Properties", properties))
		{
			...
			if(properties->GetValue("RootedAngleMax", m_rootedAngleDotMin)) // Max in lua (angle) == min in cos angle (0=1, 1=180)
			{
				...
				m_rootedAngleDotMin = (float)fsel(m_rootedAngleDotMin, cos_tpl(DEG2RAD(clamp_tpl<float>(m_rootedAngleDotMin,0.f,180.f))), -1.f);
			}
			if(properties->GetValue("RootedAngleMin", m_rootedAngleDotMax))
			{
				// Specified in degrees of half cone from the front. 0-180
				m_rootedAngleDotMax = (float)fsel(m_rootedAngleDotMax, cos_tpl(DEG2RAD(clamp_tpl<float>(m_rootedAngleDotMax,0.f,180.f))), 1.f);
			}
...
Vec3 CEnvironmentalWeapon::CalculateImpulse_Object( const Vec3& hitLoc, const Vec3& nImpactDir, EntityId attackerId, const float victimObjectMass ) const 
{
	...
	if(victimObjectMass && victimObjectMass < g_pGameCVars->pl_pickAndThrow.objectImpulseLowMassThreshold)
	{
		...
		float tVal  = (victimObjectMass * fres(s_objectImpulseLowMassThreshold)); // [0.0f,1.0f] (equivalent to 'victimObjectMass / g_pGameCVars->pl_pickAndThrow.objectImpulseLowMassThreshold' )
...
void CEnvironmentalWeapon::ApplyImpulse(const HitInfo& hitInfo, const EntityId victimEntityId) const
{
	...
	if (pVictimPhysicalEntity)
	{
		...
		// OBJECTS
		else if(IComponentPhysics* pPhysicsProxy = (IComponentPhysics*)pEntity->GetComponent<IComponentPhysics>().get())
		{
			const Vec3 vImpulseVec  = (hitInfo.impulseScale * hitInfo.dir); 
...
void CEnvironmentalWeapon::PostInitClient (ChannelId channelId)
{
	...
	if(gEnv->bServer && prevOwnerId != newOwnerId)
	{
		...
		if(newOwnerId)
		{
			...
			if(pActor) 
			{
				...
				// TODO: does this still make sense and is needed?

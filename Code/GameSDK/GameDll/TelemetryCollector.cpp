	...
	m_transfersCounter(0)
{
	...
	AZ::IO::HandleType fileHandle = gEnv->pCryPak->FOpen(m_telemetryMemoryLogPath.c_str(), "at");
	allowFileAccess.End();

	if (fileHandle != AZ::IO::InvalidHandle)
	{
		gEnv->pCryPak->FPrintf(fileHandle, "<sheet>\n");
		gEnv->pCryPak->FPrintf(fileHandle, "</sheet>\n");
...
bool CTelemetryCollector::UploadFileForPreviousSession(const char *inFileName, const char *inRemoteFileName)
{
	...
	if (gEnv->pCryPak->IsFileExist(inFileName, ICryPak::eFileLocation_OnDisk))
	{
		...
		if (gEnv->pCryPak->IsFileExist(k_hintFileName, ICryPak::eFileLocation_OnDisk))
		{
			...
			CDebugAllowFileAccess allowFileAccess;
			AZ::IO::HandleType fileHandle = gEnv->pCryPak->FOpen(k_hintFileName, "rt");
			allowFileAccess.End();
			...
			if (fileHandle != AZ::IO::InvalidHandle)
			{
				...
				gEnv->pCryPak->FRead(hintFileData, sizeof(hintFileData), fileHandle);
				gEnv->pCryPak->FClose(fileHandle);
	...
	if (gEnv->pCryPak->IsFileExist(inFileName, ICryPak::eFileLocation_OnDisk))
	{
		...
		if (gEnv->pCryPak->IsFileExist(k_hintFileName, ICryPak::eFileLocation_OnDisk))
		{
			...
			if (fileHandle != AZ::IO::InvalidHandle)
			{
				...
				int readSize = gEnv->pCryPak->FRead(hintFileData, sizeof(hintFileData), fileHandle);
				gEnv->pCryPak->FClose(fileHandle);
...
void CTelemetryCollector::CheckForPreviousSessionCrash()
{
	...
	CDebugAllowFileAccess allowFileAccess;
	AZ::IO::HandleType crashCheckFile = AZ::IO::InvalidHandle;
	gEnv->pFileIO->Open(crashCheckFileName, AZ::IO::OpenMode::ModeWrite | AZ::IO::OpenMode::ModeText, crashCheckFile);

	allowFileAccess.End();

	if (crashCheckFile != AZ::IO::InvalidHandle)
	{
...
bool CTelemetryCollector::InitService()
{
	...
	if (pLobby)
	{
		ICryLobbyService* pLobbyService = FakeGetLobby()->GetLobbyService();
...
//	if (m_pTelemetry==NULL)
	...
	{
		const char *hostName = "localhost"; // m_pTelemetry->GetServerName();		// HTTP 1.1 requires hostname
		const int hostPort = 8000; // m_pTelemetry->GetPort();		// And port if it's not 80
	...
	const int				inHeaderLength)
{
	...
	if (shouldSubmit)
	{
		...
		CDebugAllowFileAccess allowFileAccess;
		AZ::IO::HandleType file = pak->FOpen(inLocalFilePath,"rb",ICryPak::FLAGS_PATH_REAL|ICryPak::FOPEN_ONDISK);
		allowFileAccess.End();
...
{
	CDebugAllowFileAccess allowFileAccess;
	m_fileHandle = gEnv->pCryPak->FOpen(m_fileName.c_str(), "wt");
...
CTelemetrySaveToFile::~CTelemetrySaveToFile()
{
	if ((m_fileHandle != AZ::IO::InvalidHandle) && (gEnv->pCryPak))
	{
		...
		case eTS_EndOfStream:
			{
				...
				{
					if (m_fileHandle != AZ::IO::InvalidHandle)
						written = gEnv->pCryPak->FWrite(pOutBuffer, 1, toWrite, m_fileHandle);
	...
	int					*pOutWritten)
{
	...
	AZ::IO::HandleType pFile = pPak->FOpen(m_localFilePath.c_str(),"rb",ICryPak::FLAGS_PATH_REAL|ICryPak::FOPEN_ONDISK);
	allowFileAccess.End();
...
void CTelemetryCollector::CreateStatoscopeStream()
{
	...
	if (gEnv->pStatoscope->IsLoggingForTelemetry() && 0 /*m_pTelemetry */)
	{
...
//		success=m_pTelemetry ? m_pTelemetry->UploadData(pData) : false;
	...
	if ((!success && recording==k_recording_ifServiceUnavailable) || recording==k_recording_always)
	{
		...
		CDebugAllowFileAccess allowFileAccess;
		AZ::IO::HandleType file = pak->FOpen(path,"wb",ICryPak::FLAGS_PATH_REAL);
		allowFileAccess.End();
...
void CTelemetryCollector::OutputTelemetryServerHintFile()
{
	...
	CDebugAllowFileAccess allowFileAccess;
	AZ::IO::HandleType file = gEnv->pCryPak->FOpen( k_hintFileName,"wt" );
	allowFileAccess.End();
...
void CTelemetryCollector::OutputMemoryUsage(const char *message, const char *newLevelName)
{
	...
	CDebugAllowFileAccess allowFileAccess;
	AZ::IO::HandleType fileHandle = gEnv->pCryPak->FOpen(m_telemetryMemoryLogPath.c_str(), "r+");
	allowFileAccess.End();

	if (fileHandle)
	{
		int fileSize = gEnv->pCryPak->FGetSize(fileHandle);
		gEnv->pCryPak->FSeek(fileHandle, fileSize - 10, SEEK_SET);
		gEnv->pCryPak->FPrintf(fileHandle, "  <row timestamp=\"%s\" type=\"%s\" map1=\"%s\" map2=\"%s\" memoryused=\"%f\">\n", timeStr.c_str(), message, mapName, newLevelName, cpuMemUsedInMB);
		gEnv->pCryPak->FPrintf(fileHandle, "</sheet>\n");
		//gEnv->pCryPak->FPrintf(fileHandle, "%s - %s - %s -> %s - %f\n", timeStr.c_str(), message, mapName, newLevelName, cpuMemUsedInMB);
		gEnv->pCryPak->FClose(fileHandle);
	}
...
#define FIXED_BUFFER_SIZE (10 * 1024)
	...
	CDebugAllowFileAccess allowFileAccess;
	AZ::IO::HandleType file = gEnv->pCryPak->FOpen(filename, "wt");
	allowFileAccess.End();

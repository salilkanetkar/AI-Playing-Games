...
// Description : This module keeps track of all hazards in the world and will notify 
	...
	const Vec3& hazardNormal)
{
	...
	// Debug visualization.
	//gEnv->pGame->GetIGameFramework()->GetIPersistentDebug()->Begin("HazardDebugGraphics", false);
	//gEnv->pGame->GetIGameFramework()->GetIPersistentDebug()->AddSphere(estimatedHazardPos, 0.3f, Col_DarkGray, 10.0f);
...
{
	IPersistentDebug *debugRenderer = gEnv->pGame->GetIGameFramework()->GetIPersistentDebug();
...
void HazardModule::RenderDebugProcessContainer(
	const CONTAINER& container, IPersistentDebug& debugRenderer) const
{

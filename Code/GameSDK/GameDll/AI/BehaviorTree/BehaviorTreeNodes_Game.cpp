...
#define MELEE_FRAGMENT_NAME "melee"
	...
	public:
		struct RuntimeData : public IMeleeActionListener
		{
				...
				if (!hasBeenKnockedDown)
				{
					if (pCollider)
					{
						...
						if (pEntity)
						{
							...
							else if (IComponentPhysics* pPhysicsProxy = static_cast<IComponentPhysics*>(pEntity->GetComponent<IComponentPhysics>().get()))
							{
			...
			bool PerformCylinderTest(const Vec3 &pos, const Vec3 &dir, const float hitRange)
			{
				...
				if(s_debug)
				{
					IPersistentDebug* persistentDebug = g_pGame->GetIGameFramework()->GetIPersistentDebug();
				...
				fIntersectionsNumber = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(primitives::cylinder::type, &cylinder, Vec3(ZERO), 
					ent_living | ent_rigid, &contacts, 0, geom_colltype_player, &params, PhysicsForeignData(), 0, &pIgnore, pIgnore ? 1 : 0);
	...
	protected:	
		virtual Status Update(const UpdateContext& context) override
		{
			...
			if (IComponentPhysics* physicsProxy = (IComponentPhysics*)targetEntity->GetComponent<IComponentPhysics>().get())
			{

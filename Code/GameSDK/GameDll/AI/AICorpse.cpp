...
DECLARE_DEFAULT_COMPONENT_FACTORY(CAICorpse, CAICorpse)
CAICorpse::CAICorpse()
{
}
...
void CAICorpseManager::Update( const float frameTime )
{
	...
	for(uint32 i = m_lastUpdatedCorpseIdx; i < corpsesEndIdx; ++i)
	{
		...
		if(pCorpseEntity != NULL)
		{
			...
			{
				IComponentPhysics* pCorpsePhysicsProxy = static_cast<IComponentPhysics*>(pCorpseEntity->GetComponent<IComponentPhysics>().get());

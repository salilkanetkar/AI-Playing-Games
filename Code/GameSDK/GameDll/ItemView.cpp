...
void CItem::CopyRenderFlags(IEntity *pOwner)
{
	...
	if (pRenderNode)
	{
		IComponentRender *pOwnerRenderProxy = (IComponentRender *)pOwner->GetComponent<IComponentRender>().get();
		IRenderNode *pOwnerRenderNode = pOwnerRenderProxy?pOwnerRenderProxy->GetRenderNode():NULL;
		if (pOwnerRenderNode)
		{
			int viewDistRatio = pOwnerRenderNode->GetViewDistanceMultiplier();
			...
			pRenderNode->SetViewDistanceMultiplier(viewDistRatio);

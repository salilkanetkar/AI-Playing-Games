...
//------------------------------------------------------------------------
DECLARE_DEFAULT_COMPONENT_FACTORY(CProjectile, CProjectile)
...
bool CProjectile::Init(IGameObject *pGameObject)
{
	...
	IComponentRender *pProxy = static_cast<IComponentRender *>(pEntity->GetComponent<IComponentRender>().get());
	if (pProxy && pProxy->GetRenderNode())
	{
		pProxy->GetRenderNode()->SetViewDistanceMultiplier(255);
...
//	}
	...
	if (g_pGameCVars->g_ProjectilePathDebugGfx)
	{
		if (IsGrenade())
		{
			IPersistentDebug * dbg = g_pGame->GetIGameFramework()->GetIPersistentDebug();
...
//------------------------------------------------------------------------
IComponentAudio *CProjectile::GetAudioProxy()
{
	IComponentAudio *pIComponentAudio = static_cast<IComponentAudio *>(GetEntity()->GetComponent<IComponentAudio>().get());

	if (!pIComponentAudio)
		pIComponentAudio=static_cast<IComponentAudio *>(GetEntity()->GetOrCreateComponent<IComponentAudio>().get());

	assert(pIComponentAudio);
...
//------------------------------------------------------------------------
void CProjectile::SerializeSpawnInfo( TSerialize ser, IEntity * )
{
...
void CProjectile::DoCollisionDamage(EventPhysCollision* pCollision, IEntity *pTarget)
{
	...
	damage = static_cast<float>(fsel(pCollisionParams->damageLimit, min(pCollisionParams->damageLimit, damage), damage));

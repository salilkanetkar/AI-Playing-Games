...
// Description : Player Mind-Game Beam effect// 
...
m_LightningID(GameSDK::CLightningGameEffect::TIndex(-1))
	, m_EffectConfig(NULL)
	, m_RestartDelay(0.0f)
{
}
...
void CPlayerMindGameBeamEffectLinkedEffect::BetweenPoints(const Vec3& startPos, const Vec3& endPos)
{
    ...
    GameSDK::CLightningGameEffect* lightningGameEffect = g_pGame->GetLightningGameEffect();
	...
	if (m_LightningID == -1)
	{
		...
		m_LightningID = lightningGameEffect->TriggerSpark(
            ...
            GameSDK::CLightningGameEffect::STarget(startPos), GameSDK::CLightningGameEffect::STarget(endPos));
	...
	{
        lightningGameEffect->SetEmitter(m_LightningID, GameSDK::CLightningGameEffect::STarget(startPos));
        lightningGameEffect->SetReceiver(m_LightningID, GameSDK::CLightningGameEffect::STarget(endPos));
	}
}

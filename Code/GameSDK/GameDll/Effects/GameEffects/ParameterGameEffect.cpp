...
// Description : Manages the value of the game effects' parameters when more than one module is trying to
...
void CParameterGameEffect::SetActive(bool isActive)
{
	...
	if (bActive != isActive)
	{
		if (isActive)
		{
            GameSDKCGameEffect::SetActive(true);
		}
		else
		{
			...
			{
                GameSDKCGameEffect::SetActive(false);
			}
			else
			{

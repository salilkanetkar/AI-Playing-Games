...
void CPlayerHealthGameEffect::Update(float frameTime)
{
	...
	if (pClientActor)
	{
		...
		else
		{
			...
			{
				killerHealthPercentage = currentHealth * (float)fres(maxHealth);
...
void CPlayerHealthGameEffect::SetActive(bool isActive)
{
	...
	pRenderer->EF_SetPostEffectParam("ScreenBlood_Amount", 0.0f);
	pRenderer->EF_SetPostEffectParam("FilterRadialBlurring_Amount", 0.0f);
...
void CPlayerHealthGameEffect::Start()
{
	...
	{
		gEnv->pRenderer->EF_SetPostEffectParam("HUDHitEffect_HealthLevel", 1.0f);
...
void CPlayerHealthGameEffect::OnKill()
{
	...
	{
		gEnv->pRenderer->EF_SetPostEffectParam("HUDHitEffect_HealthLevel", 0.0f);
	}
}
...
void CPlayerHealthGameEffect::UpdateHealthReadibility(float effectIntensity, float frameTime)
{
	...
	pRenderer->EF_SetPostEffectParam("ScreenBlood_Amount", m_screenBloodIntensity);
	...
	if (m_radialBlurIntensity > 0.05f)
	{
		...
		pRenderer->EF_SetPostEffectParam("FilterRadialBlurring_Amount", radialBlurAmount);
		pRenderer->EF_SetPostEffectParam("FilterRadialBlurring_ScreenPosX", 0.5f);
		pRenderer->EF_SetPostEffectParam("FilterRadialBlurring_ScreenPosY", 0.5f);
		pRenderer->EF_SetPostEffectParam("FilterRadialBlurring_Radius", 1.5f);
		if (m_useBlurMask)
		{

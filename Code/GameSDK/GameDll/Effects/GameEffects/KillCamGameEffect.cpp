...
void CKillCamGameEffect::Initialise(const SGameEffectParams* gameEffectParams)
{
    ...
    GameSDKCGameEffect::Initialise(&killCamGameEffectParams);
	...
	if(IsFlagSet(GAME_EFFECT_ACTIVE))
	{
		...
		if(data.chromaFadeTimeInv>0.f)
		{
			...
			if(scale>0.01f)
			{
				...
				gEnv->pRenderer->EF_SetPostEffectParam("FilterKillCamera_Active", 1.f);
				gEnv->pRenderer->EF_SetPostEffectParamVec4("FilterKillCamera_ChromaShift", chroma*scale);
				gEnv->pRenderer->EF_SetPostEffectParamVec4("FilterKillCamera_ColorScale", color*scale);
			}
			else
			{
				gEnv->pRenderer->EF_SetPostEffectParamVec4("FilterKillCamera_ChromaShift", Vec4(0.f,0.f,0.f,0.f));
				gEnv->pRenderer->EF_SetPostEffectParamVec4("FilterKillCamera_ColorScale", Vec4(0.f,0.f,0.f,0.f));
			}
		}
		...
		if(data.fadeOutTimeInv>0.f)
		{
			...
			{
				m_remainingTime = (float)fsel(m_remainingTime-frameTime,m_remainingTime-frameTime,0.f);
				...
				gEnv->pRenderer->EF_SetPostEffectParam("Global_User_Brightness", brightness);
			}
		}
	}
}
...
void CKillCamGameEffect::SetActive(bool isActive)
{
	if(s_data[m_currentMode].isInitialised)
	{
        ...
        GameSDKCGameEffect::SetActive(isActive);
	}
}//-------------------------------------------------------------------------------------------------
...
void CKillCamGameEffect::LoadStaticModeData( const IItemParamsNode* paramNode, SEffectData& data )
{
	if(paramNode)
	{
		...
		if(paramNode->GetAttribute("chromaFadeTime", data.chromaFadeTimeInv)) { data.chromaFadeTimeInv=fres(data.chromaFadeTimeInv); }

		// Fade Out Time.
		data.fadeOutTimeInv = 0.f;
		if(paramNode->GetAttribute("fadeOutTime", data.fadeOutTimeInv)) { data.fadeOutTimeInv=fres(data.fadeOutTimeInv); }

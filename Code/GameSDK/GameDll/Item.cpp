...
//------------------------------------------------------------------------
DECLARE_DEFAULT_COMPONENT_FACTORY(CItem, CItem)
...
namespace 
{
	...
	{
		IGameObject *cgo = pPlayer->GetGameObject();
		IWorldQuery *wq = cgo ? cgo->GetWorldQuery() : 0;

		EntityId targetEntityId = wq ? wq->GetLookAtEntityId() : 0;
		...
		case eIPhys_NotPhysicalized:
			{
				...
				{
					IComponentPhysics *pPhysicsProxy = GetPhysicalProxy();
					if (pPhysicsProxy)
					{
						SEntityPhysicalizeParams params;
						params.type = PE_NONE;
	...
	case ENTITY_EVENT_TIMER:
		{
			...
			case eIT_Flying:
				...
				IComponentPhysics *pPhysics = GetPhysicalProxy();
				if (pPhysics)
...
{
	IComponentRender* pProxy = GetRenderProxy();
...
{
	return static_cast<CActor *>(m_pGameFramework->GetIActorSystem()->GetActorByChannelId(gEnv->pNetwork->GetServerChannelId()));
}
...
void CItem::CloakSync(bool fade)
{
	...
	IComponentRender* pOwnerRP = (IComponentRender*)pOwner->GetComponent<IComponentRender>().get();
	if (pOwnerRP)
	{
...
void CItem::SetIgnoreHeat( bool ignoreHeat )
{
	...
	IComponentRender* pItemRenderProxy = static_cast<IComponentRender*>(GetEntity()->GetComponent<IComponentRender>().get());
	if(pItemRenderProxy)
	{
	...
	const int numAccessories = m_accessories.size();
	IComponentRender* pItemAccessoryRenderProxy(NULL);
	for(int i = 0; i < numAccessories; ++i)
	{
		IEntity* pAccessory = gEnv->pEntitySystem->GetEntity(m_accessories[i].accessoryId);
		if(pAccessory && (pItemAccessoryRenderProxy = static_cast<IComponentRender*>(pAccessory->GetComponent<IComponentRender>().get())))
		{

	...
	else
	{
...

		{
			...
			{ // Shallow water speed slowdown
				...
				shallowWaterMultiplier = (float)fsel(-relativeBottomDepth, 1.0f, shallowWaterMultiplier);
		...
		if (isPlayer)
		{
			...
			const float divisorH = (float)fsel(-h, 1.0f, h);
			const float divisorV = (float)fsel(-v, 1.0f, v);

			const float invV = fres(divisorV);
			const float invH = fres(divisorH);

			const float slopeAngleHor = 10.0f;
			const float slopeAngleVer = 50.0f;
			float slopeAngleFraction = clamp_tpl((slopeAngleCur - slopeAngleHor) * fres(slopeAngleVer - slopeAngleHor), 0.0f, 1.0f);
			...
			const float hMultiplier = (float)fsel(-h, 1.0f, s * invH);
			const float vMultiplier = (float)fsel(-v, 1.0f, c * invV);
			...
			const float fSlopeNormalLengthSafe = (float)fsel(fSlopeNormalLength - 0.000001f, fSlopeNormalLength, 1.0f);
			modifiedSlopeNormal = modifiedSlopeNormal * fres(fSlopeNormalLengthSafe);
			...
			alignment *= (float)fsel(-fabsf(stats.onGround), LERP(0.7f, 1.0f, 1.0f - clamp_tpl(modifiedSlopeNormal.z * 100.0f, 0.0f, 1.0f)), 1.0f);
		...
		const float fVelocityMultiplier = (float)fsel(fNewSpeed - 22.0f, fres(fNewSpeed+FLT_EPSILON) * 22.0f, 1.0f);

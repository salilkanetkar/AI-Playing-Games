...
void CWeaponFPAiming::Update(const SParams_WeaponFPAiming &params)
{
	...
	if (m_enabled && frameTime > 0.0f)
	{
		...
		{
			float factor = (float)fsel(params.aimDirection.z, STAP_MF_Up, -STAP_MF_Down);
		...
		{
			float factor = (float)fsel(inputRot.z, STAP_MF_Left, -STAP_MF_Right);
		...
		else
		{
			...
			float easeFactor = (float)fsel(fabs_tpl(m_interpVert) - fabs_tpl(vertInterp), decStep, incStep);
			easeFactor = clamp_tpl(easeFactor, 0.0f, 1.0f);
			vertInterp  = m_interpVert = (vertInterp * easeFactor) + (m_interpVert * (1.0f - easeFactor));

			easeFactor = (float)fsel(fabs_tpl(m_interpHoriz) - fabs_tpl(horizInterp), decStep, incStep);
			easeFactor = clamp_tpl(easeFactor, 0.0f, 1.0f);
		...
		interpFront *= (float)fsel(interpFront, STAP_MF_Front, STAP_MF_Back) * strafeFactor;
		...
		interpSide *= (float)fsel(interpSide, STAP_MF_StrafeLeft, STAP_MF_StrafeRight) * strafeFactor;

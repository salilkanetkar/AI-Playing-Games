...
DECLARE_DEFAULT_COMPONENT_FACTORY(CGameRules, CGameRules)
...
#undef GAMERULES_MODULE_LIST_FUNC
	...
	// TODO: HACK_NET_OUT: do we have to do anything special to make this happen w/ GridMate?
...
//------------------------------------------------------------------------
void CGameRules::InitClient (ChannelId channelId)
{
	if( m_pVTOLVehicleManager )
	{
...
//------------------------------------------------------------------------
void CGameRules::PostInitClient (ChannelId channelId)
{
  ...
  case ENTITY_EVENT_POST_SERIALIZE:
	...
	case ENTITY_EVENT_START_GAME:
	{
		...
		if(gEnv->IsClient())
		{
			...
			// CryAction used to bind the player action map as well. This is no longer true, it was part of the networking code that
			// is now gone. Try and do it here instead.
			pAM = pActionMapMan->GetActionMap("player");
			if (pAM) {
				IActor* pActor = g_pGame->GetIGameFramework()->GetClientActor();
				if (pActor)
					pAM->SetActionListener(pActor->GetEntityId());
			}
	...
	case ENTITY_EVENT_ENTER_SCRIPT_STATE:
		...
		IComponentScript *pScriptProxy=static_cast<IComponentScript *>(GetEntity()->GetComponent<IComponentScript>().get());
		if (pScriptProxy)
		{
...
//------------------------------------------------------------------------
IActor *CGameRules::GetActorByChannelId(ChannelId channelId) const
{
...
//------------------------------------------------------------------------
bool CGameRules::ShouldKeepClient(ChannelId channelId, EDisconnectionCause cause, const char *desc) const
{
...
//------------------------------------------------------------------------
bool CGameRules::OnClientConnect(ChannelId channelId, bool isReset)
{
	...
	if (gEnv->bServer && gEnv->bMultiplayer)
	{
		...
		{
			if(!playerName.empty())
				CallScript(m_serverStateScript, "OnClientConnect", IntToHandle(channelId), isReset, playerName.c_str());
			else
				CallScript(m_serverStateScript, "OnClientConnect", IntToHandle(channelId), isReset);
		}
	}
	else if (!hasSetupModule)
	{
		CallScript(m_serverStateScript, "OnClientConnect", IntToHandle(channelId));
	}
	if (hasSetupModule)
	{
	...
	if (pActor)
	{
		...
		m_pGameplayRecorder->Event(pActor->GetEntity(), GameplayEvent(eGE_Connected, 0, 0.0f /*m_pGameFramework->IsChannelOnHold(channelId)?1.0f:0.0f*/, (void*)status));
...
//------------------------------------------------------------------------
void CGameRules::OnClientDisconnect(ChannelId channelId, EDisconnectionCause cause, const char *desc, bool keepClient)
{
...
//------------------------------------------------------------------------
bool CGameRules::OnClientEnteredGame(ChannelId channelId, bool isReset)
{ 
...
//------------------------------------------------------------------------
IActor *CGameRules::SpawnPlayer(ChannelId channelId, const char *name, const char *className, const Vec3 &pos, const Ang3 &angles)
{ 
	if (!gEnv->bServer)
		return 0;
...
// Disabling this logic for dedicated server, it does not work with the new networking system.
/*
...
#if HACK_NET_OUT
	// TODO: does this concent exist any longer?
...
#if HACK_NET_OUT_NICKNAME
		// TODO: how does this work with GridMate?
...
{
#if HACK_NET_OUT_DETERMINE_IN_GAME
	//TODO: figure out how to tell if a player is in game with GridMate
...
//------------------------------------------------------------------------
bool CGameRules::IsChannelInGame(ChannelId channelId) const
{
#if HACK_NET_OUT_CHANNEL_IN_GAME
	// TODO: figure out how to do this with GridMate
...
void CGameRules::SetTeam(int teamId, EntityId entityId, bool clientOnly /*= false*/)
{
	...
	if(SetTeam_Common(teamId, entityId, bIsPlayer))
	{
		if(bIsPlayer)
		{
			ChannelId channelId=GetChannelId(entityId);
...
//------------------------------------------------------------------------
int CGameRules::GetChannelTeam(ChannelId channelId) const
{
...
//------------------------------------------------------------------------
void CGameRules::SendTextMessage(ETextMessageType type, const char *msg, unsigned int to, ChannelId channelId,
																 const char *p0, const char *p1, const char *p2, const char *p3)
{
...
//------------------------------------------------------------------------
EntityId CGameRules::SetChannelForMigratingPlayer(const char* name, ChannelId channelID)
{
...
void CGameRules::SendNetConsoleCommand(const char *msg, unsigned int to, ChannelId channelId)
{
...
//------------------------------------------------------------------------
void CGameRules::OnUserLeftLobby( ChannelId channelId )
{
...
bool CGameRules::IsInsideForbiddenArea(const Vec3& pos, bool doResetCheck, IEntity** ppArea )
{
	...
	for(int i = 0; i < helperCount; i++)
	{
		...
		if(pEntity)
		{
			IComponentArea *pAreaProxy = (IComponentArea*)pEntity->GetComponent<IComponentArea>().get();
			if(pAreaProxy)
			{
...
void CGameRules::ForceBalanceTeams()
{
	...
	if (pGameLobby && pGameLobby->UseLobbyTeamBalancing() && !pGameLobby->IsGameBalanced())
	{
		...
		{
			ChannelId channelId = m_channelIds[i];
			IActor *pActor = GetActorByChannelId(channelId);
			if (pActor)
			{

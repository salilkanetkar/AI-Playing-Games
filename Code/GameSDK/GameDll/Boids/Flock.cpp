...
namespace GameSDK
{
    ...
    void CFlock::UpdateBoidsViewDistRatio()
    {
        ...
        if (pRenderComponent)
        {
            m_nViewDistRatio = pRenderComponent->GetRenderNode()->GetViewDistanceMultiplier();
        }
        ...
        for (Boids::iterator it = m_boids.begin(); it != m_boids.end(); ++it)
        {
            ...
            if (pBoidEntity)
            {
                ...
                if (pRenderComponent)
                {
                    pRenderComponent->GetRenderNode()->SetViewDistanceMultiplier(m_nViewDistRatio * 100.f);
                }
            }
        }
    }
    ...
    void CFlock::Update(CCamera* pCamera)
    {
        ...
        */
        ...
        if (pRenderComponent)
        {
            if (pRenderComponent->GetRenderNode()->GetViewDistanceMultiplier() != m_nViewDistRatio)
    ...
    void CFlock::CreateBoids(SBoidsCreateContext& ctx)
    {
        ...
        if (ctx.boidsCount == 0)
            return;
        /*	Disable cache as GameCore does not provide caching functionality that GameSDK does.
                https://issues.labcollab.net/browse/LMBR-5774

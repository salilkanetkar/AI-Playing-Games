...
void CRecordingSystem::HideEntityKeepingPhysics(IEntity *pEntity, bool hide)
{
	...
	// TODO:
	// Need to figure out a new way to do this. THink we have to explicitly list the components, since there
	// is no way now to iterate over the components?
	//
#if 0
	...
	for (int i = 0; i < ENTITY_PROXY_LAST; i++)
	{
		...
		{
			IComponent *pProxy = pEntity->GetProxy(proxyType);
			if (pProxy)
...
void CRecordingSystem::SetPlayBackCameraView(bool bSetView)
{
	...
	if(pActiveView)
	{
		...
		if(pKillerReplayActor)
		{
			...
			{
				CGameObject* pKillerReplayGameObject = (CGameObject*) pKillerReplayEntity->GetComponent<IComponentUser>().get();
				if(pKillerReplayGameObject)
				{
					...
					{		   
						...
						{
							CGameObject* pGameCameraLinkedToGameObject = (CGameObject*)pGameCameraLinkedToEntity->GetComponent<IComponentUser>().get();
...
void CRecordingSystem::RecordTPCharPacket(IEntity *pEntity, CActor *pActor)
{
	...
	IComponentRender* pRenderProxy = static_cast<IComponentRender*>(pEntity->GetComponent<IComponentRender>().get());
	if (pRenderProxy)
	{
...
#undef SetFlags
	...
	if (pReplayEntity)
	{
		...
		IComponentRender* pRenderProxy = static_cast<IComponentRender*>(pReplayEntity->GetComponent<IComponentRender>().get());
		if (pRenderProxy)
		{
...
void CRecordingSystem::CloakEnable(IComponentRender* pRenderProxy, bool enable, bool fade)
{
...
void CRecordingSystem::ApplyEntitySpawn(const SRecording_EntitySpawn *entitySpawn, float time)
{
	...
	if (IEntity* pEntity = gEnv->pEntitySystem->SpawnEntity(params, false))
	{
		...
		{
			if (pGameObject = (CGameObject*)pEntity->GetComponent<IComponentUser>().get())
			{
...
void CRecordingSystem::ApplyPlaySound(const SRecording_PlaySound* pPlaySound, float time)
{
	...
	//		if (pEntity)
	//		{
	//			IComponentAudioPtr pIComponentAudio = crycomponent_cast<IComponentAudioPtr>(pEntity->GetOrCreateComponent<IComponentAudio>().get());
	//			CRY_ASSERT_MESSAGE(pIComponentAudio.get(), "Failed to create sound proxy");
	//			if(pIComponentAudio)
	//			{
	//				pIComponentAudio->PlaySound(pSound);
	//			}
	//		}
	//		else
	//		{
...
EntityId CRecordingSystem::EntityIdToNetId(EntityId entityId)
{
	//INetContext* pNetContext = gEnv->pGame->GetIGameFramework()->GetNetContext();
	if (gEnv && gEnv->pNetwork)
	{
		return gEnv->pNetwork->LocalEntityIdToServerEntityId(entityId);
...
/*static*/ EntityId CRecordingSystem::NetIdToEntityId(EntityId netId)
{
	//INetContext* pNetContext = gEnv->pGame->GetIGameFramework()->GetNetContext();
	if (gEnv && gEnv->pNetwork)
	{
		return gEnv->pNetwork->ServerEntityIdToLocalEntityId(netId);
...
/*static*/ void CRecordingSystem::RemoveEntityPhysics( IEntity& entity )
{
	...
	// Set ViewDistRatio.
	if(IComponentRender* pRenderProxy = (IComponentRender*)entity.GetComponent<IComponentRender>().get())
	{

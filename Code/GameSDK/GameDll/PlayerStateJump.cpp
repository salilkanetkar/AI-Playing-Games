...
void CPlayerStateJump::Landed(CPlayer& player, const bool isHeavyWeapon, float fallSpeed)
{
	...
	if (effectId != InvalidEffectId)
	{
		...
		float landFallParamVal = (float)fsel( -(fallSpeed - 7.5f), 0.25f, 0.75f);
...
bool CPlayerStateJump::UpdateCommon( CPlayer& player, const bool isHeavyWeapon, const Vec3 &move, float frameTime, Vec3* pDesiredVel )
{
	...
	if (actorPhysics.velocity*actorPhysics.gravity>0.0f)
	{
		...
		m_startFallingHeight= (float)fsel(-player.m_stats.fallSpeed, fHeightofEntity, max(m_startFallingHeight, fHeightofEntity));
	...
	m_jumpLock = (float)fsel(-fabsf(player.m_stats.inAir), max(0.0f, m_jumpLock - frameTime), m_jumpLock);

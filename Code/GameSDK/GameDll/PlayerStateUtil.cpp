...
void CPlayerStateUtil::CalculateGroundOrJumpMovement( const CPlayer& player, const SActorFrameMovementParams &movement, const bool bigWeaponRestrict, Vec3 &move )
{
	...
	{	
		...
		desiredVelocityClamped = desiredVelocityClamped * (float)fsel(-(desiredVelocityMag - 1.0f), 1.0f, invDesiredVelocityMag);

		//going back?
		if (isPlayer)	//[Mikko] Do not limit backwards movement when controlling AI.
		{
			backwardMul = (float)fsel(desiredVelocityClamped.y, 1.0f, LERP(backwardMul, player.m_params.backwardMultiplier, -desiredVelocityClamped.y));
		}
	...
	if (movement.sprint<=0.0f)
	{
		...
		if (isPlayer)
		{
			move /= (float)fsel(-(moveModule - 1.0f), 1.0f, moveModule);
		}
...
void CPlayerStateUtil::ApplyFallDamage( CPlayer& player, const float startFallingHeight, const float fHeightofEntity )
{
	...
	const float downwardsImpactSpeed = -(float)fsel(-(player.m_playerStateSwim_WaterTestProxy.GetSwimmingTimer() + 0.5f), player.GetActorPhysics().velocityUnconstrainedLast.z, 0.0f);
	...
	if ((downwardsImpactSpeed > 0.0f) && (fallDist > MIN_FALL_DAMAGE_DISTANCE))
	{
		...
		float velFraction = (float)fsel(-(velFatal - velSafe), 1.0f , (downwardsImpactSpeed - velSafe) * (float)fres(velFatal - velSafe));
		...
		if (velFraction > 0.0f)
		{
			...
			const float maxDamage = (float)fsel(velFraction - 1.0f, maxHealth, max(0.0f, (gEnv->bMultiplayer?maxHealth:currentHealth) - healthCVars.fallDamage_health_threshold));

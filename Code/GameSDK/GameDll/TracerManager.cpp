...
{
	frameTime							= (float)fsel(-m_age, 0.002f, frameTime);
	const float tracerAge	= (float)fsel(-m_age, 0.002f, m_age+frameTime);
	...
	const Vec3 dir = maxTravelledDistance * (float)fres(dist);
	Vec3 pos = m_pos;
	...
	if (!(m_tracerFlags & kTracerFlag_dontTranslate))
  {
    ...
    const float speed = m_speed * (float)fsel(sqrRadius - cameraDistance, 0.35f + (cameraDistance/(sqrRadius*2)), 1.0f); //Slow down tracer when near the player

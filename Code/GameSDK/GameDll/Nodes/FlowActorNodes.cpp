			...
			if (IsPortActive(pActInfo, eIP_Trigger))
			{
				...
				else if (!m_errorLogSent)
				{
					...
					GameWarning("[flow] Actor:AliveCheck - flowgraph entity: %d:'%s' - no entity or entity is not an actor. Entity: %d:'%s'", 
						static_cast<EntityId>(pActInfo->pGraph->GetGraphEntity( 0 )),
						pGraphEntity ? pGraphEntity->GetName() : "<NULL>",
						pActInfo->pEntity ? pActInfo->pEntity->GetId() : 0,
						pActInfo->pEntity ? pActInfo->pEntity->GetName() : "<NULL>");
					m_errorLogSent = true;
				}
			}
		}

...
CFlowConvoyNode::CFlowConvoyNode( SActivationInfo * pActInfo )
{
	...
	// Protect against an uninitialized SActivationInfo object (e.g. when called by CFlowSystem::RegisterNodeInfo()).
	if (pActInfo->pGraph)
	{
...
void CFlowConvoyNode::DiscoverConvoyCoaches(IEntity *pEntity)
{
  ...
  //  pEntity->SetConvoyEntity();
		...
		//tc.m_pEntitySoundsProxy = (IComponentAudio*) tc.m_pEntity->GetOrCreateComponent<IComponentAudio>().get();
...
//{
//	//IComponentAudio* pIComponentAudio = m_coaches[coachIndex].m_pEntitySoundsProxy;
...
//
//	//	if (pIComponentAudio->PlaySound(pSound, Vec3Constants<float>::fVec3_Zero, Vec3Constants<float>::fVec3_OneY))
//	//		ID = pSound->GetId();
//	//}
//	
//	return ID;

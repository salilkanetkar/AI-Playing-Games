...
//	lAnimPos = pCharacter->GetISkeleton()->GetAbsJointByID(endBoneID).t - pOwner->GetSlotLocalTM (characterSlot, false).GetTranslation ();
  ...
  assert(!_isnan(lAnimPos.len2()));
	...
	if (blendTime>0.001f)
	{
		...
		blendTime = (float)fsel(-fabsf(blendTime - 0.0011f), 0.0f, blendTime);
...
// CACtor is abstract, so we don't declare a factory
//DECLARE_DEFAULT_COMPONENT_FACTORY(CActor,CActor)
//------------------------------------------------------------------------
CActor::CActor()
...
bool CActor::Init( IGameObject * pGameObject )
{
	...
	// Add actor to the ActorSystem; hopefully we can get rid of this dependancy at some point.
	g_pGame->GetIGameFramework()->GetIActorSystem()->AddActor(entityId, this);
	...
	// What is the right way to do this in the new system?
	// Is ClientActorId set by this point? How could we set it?
	// Is EntityId set?
	...
	// Designate this actor as the local player if our channel owns it.
	// This is only needed because GameCore actors aren't spawned through
	// the CryActorSystem.
	//// 
	if (pGameObject->GetChannelId() == gEnv->pNetwork->GetLocalChannelId())
	{
		//m_Actor->SetIsAuthority(true);
		g_pGame->PlayerIdSet(entityId);
		gEnv->pGame->GetIGameFramework()->SetClientActor(entityId);
	}

	if (gEnv->bServer)
	{
		// HACK: This is a hack right now to see if the actor has an owner, so we can
		// bind with its parent.

		EntityId ownerId = INVALID_ENTITYID;

		if (IScriptTable* scriptTable = GetEntity()->GetScriptTable())
		{
			SmartScriptTable properties;
			if (scriptTable->GetValue("Properties", properties))
			{
				properties->GetValue("OwnerId", ownerId);
			}
		}

		// We have to bind to network.
		if (ownerId != INVALID_ENTITYID)
		{
			GetGameObject()->BindToNetworkWithParent(eBTNM_Normal, ownerId);
		}
		else
		{
...
{
	IEntity* pEntity = GetEntity();
	IComponentScript* pScript = static_cast<IComponentScript*>( pEntity->GetComponent<IComponentScript>().get() );

	g_pGame->GetActorScriptBind()->AttachTo(this);

	SEntitySpawnParams params;
	params.prevId = GetEntityId();
	pScript->InitComponent( pEntity, params );
...
//------------------------------------------------------------------------
void CActor::InitClient (ChannelId channelId)
{
...
//	if (IsPlayer())
	{
		IComponentRender *pRenderProxy = static_cast<IComponentRender *>(GetEntity()->GetComponent<IComponentRender>().get());
...
void CActor::CloakSyncEntity(EntityId entityId, bool bFade)
{
	...
	{
		IComponentRender *pOwnerRP = (IComponentRender*)GetEntity()->GetComponent<IComponentRender>().get();
		if (pOwnerRP)
		{
...
void CActor::SetChannelId (ChannelId id)
{
}
...
void CActor::SetHealth(float health)
{
	...
	if (!IsPlayer())
		fNewHealth = (float)fsel(g_pGameCVars->g_instantKillDamageThreshold, fsel(g_pGameCVars->g_instantKillDamageThreshold - (prevHealth - fNewHealth), fNewHealth, 0.0f), fNewHealth);
	m_health.SetHealth( (float)fsel(g_pGameCVars->g_maximumDamage, max(m_health.GetHealth() - g_pGameCVars->g_maximumDamage, fNewHealth), fNewHealth) );
#endif
...
bool CActor::NetSerialize( TSerialize ser, EEntityAspects aspect, uint8 profile, int pflags )
{
	...
	if (aspect == eEA_Physics)
	{
		...
		IComponentPhysics * pEPP = (IComponentPhysics *) GetEntity()->GetComponent<IComponentPhysics>().get();
		if (ser.IsWriting())
		{
...
void CActor::AttemptToRecycleAIActor()
{
	...
	if (removalAllowed)
	{
		...
		{
			SetHealth( 0.0f );
			IComponentPhysics *pPhysicsProxy = (IComponentPhysics*)GetEntity()->GetComponent<IComponentPhysics>().get();
			if (pPhysicsProxy)
			{
				SEntityPhysicalizeParams params;
				params.type = PE_NONE;
...
void CActor::OnSpectateModeStatusChanged( bool spectate )
{
	...
	{
		const ChannelId channelId = GetGameObject()->GetChannelId();
...
IComponent::ComponentEventPriority CActor::GetEventPriority( const int eventID ) const
{
	...
	{
	case ENTITY_EVENT_PREPHYSICSUPDATE:
		return(EntityEventPriority::User + EEntityEventPriority_Actor + (m_isClient ? EEntityEventPriority_Client : 0));
	}

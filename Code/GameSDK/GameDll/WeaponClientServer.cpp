...
#define CHECK_OWNER_REQUEST()	\
	{ \
		uint16 channelId=gEnv->pNetwork->GetServerChannelId();	\
	...
	if(NetAllowUpdate(true))
	{
		...
		if(m_fm)
		{
			if(m_shootCounter > 0 && m_netNextShot <= 0.0f)
			{
				...
				const float fFireRateForDiv = (float)fsel(fFireRateSelect, 1.0f, fRawFireRate);
				const float fNextShot				= (float)fsel(fFireRateSelect, 0.0f, m_netNextShot + (60.f * fres(fFireRateForDiv)));
...
IMPLEMENT_RMI(CWeapon, SvRequestStartFire)
{
	...
	// TODO: Figure out what is actually getting tested here. Seems like this is checking if the RMI was
	// invoked on the same host as it is running. What is the best way to do this with what we have implemnted?
#if 0
...
IMPLEMENT_RMI(CWeapon, SvRequestShoot)
{
	...
	bool ok=true;
	// If I am reading this right, it is taking pNetChannel as the key to actor that invoked the RMI.
	// I don't see a way to get this from GridMate at this time.
	CActor *pActor = 0; // GetActorByNetChannel(pNetChannel);
...
void CWeapon::EndVerificationSample(IActor * pActor, uint32 oldShotId)
{
	...
	{															//	without calling ShouldEndVerificationSample()
		...
		if(pFireMode)
		{
			...
			if(pActor->IsPlayer())
			{
				fMaxFireRate *= fres(pFireMode->GetTimeBetweenShotsMultiplier(static_cast<CPlayer*>(pActor)));
			}
...
IMPLEMENT_RMI(CWeapon, SvRequestShootEx)
{
	...
	else
	{
		if(params.predictionHandle)
		{
			...
			//g_pGame->GetGameRules()->GetGameObject()->InvokeRMI(CGameRules::ClPredictionFailed(), predictionParams, eRMI_ToClientChannel, gEnv->pNetwork->GetServerChannelId());
			g_pGame->GetGameRules()->GetGameObject()->InvokeRMI(
				CGameRules::ClPredictionFailed(),
				predictionParams,
				eRMI_ToClientChannel,
				gEnv->pNetwork->GetServerChannelId());

...
bool CEditorGame::ConfigureNetContext( bool on )
{
	...
	else if (on)
	{
		...
		gameParams.serverPort = EDITOR_SERVER_PORT;
	...
	{
		pGameFramework->EndGameContext(true);
...
void CEditorGame::OnAfterLevelLoad(const char *levelName, const char *levelFolder)
{
	...
	// New code imported from EditorGame.cpp in EmptyTemplate.
	// Appears we now need to inform the engine the level is ready to play.
	SEntityEvent startLevelEvent(ENTITY_EVENT_START_LEVEL);
	gEnv->pEntitySystem->SendEventToAll(startLevelEvent);

	m_pGame->GetIGameFramework()->MarkGameStarted();
...
void CEditorGame::OnSaveLevel()
{
	...
	if (pCurrentLevel)
	{
		...
		string levelPath("@devassets@/");
		levelPath += pLevelInfo->GetPath();
...
bool CEditorGame::BuildEntitySerializationList(XmlNodeRef output)
{
	...
	{
		IComponentFlowGraph* pFGProxy = static_cast<IComponentFlowGraph*>(pEntity->GetComponent<IComponentFlowGraph>().get());
		if(pFGProxy)
		{

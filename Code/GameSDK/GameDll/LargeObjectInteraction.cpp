...
// Description : Part of Player that controls interaction with large objects
			...
			{				
				...
				if (pPhysics && pPhysics->GetParams(&dimensions))
				{
					if (dimensions.bUseCapsule && !gEnv->bMultiplayer)
					{
						...
						float distance = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(physPrimitive.type, &physPrimitive, -vPlayerToTarget, ent_static | ent_terrain, NULL, 0, geom_colltype0 | geom_colltype_player, NULL, PhysicsForeignData(), 0, pSkipEntities, numSkipEntities);
						if (distance == 0.f)
						{
							return true;
						}

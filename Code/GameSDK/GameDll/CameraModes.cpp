...
bool CSpectatorFollowCameraMode::UpdateView( const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime )
{
	...
	const float hitDist = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(sphere.type, &sphere, worldOffset, ent_static|ent_terrain|ent_rigid|ent_sleeping_rigid,
		&pContact, 0, (geom_colltype_player<<rwi_colltype_bit) | rwi_stop_at_pierceable, 0, PhysicsForeignData(), 0, !skipList.empty() ? &skipList[0] : NULL, skipList.size());
		...
		else
		{
			...
			if( newZLength < minHeightDiff )
			{
				...
				const float newHitDist = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(sphere.type, &sphere, Vec3(0,0,minHeightDiff), ent_static|ent_terrain|ent_rigid|ent_sleeping_rigid,
					&pContact, 0, (geom_colltype_player<<rwi_colltype_bit) | rwi_stop_at_pierceable, 0, nullptr, 0, !skipList.empty() ? &skipList[0] : NULL, skipList.size());
	...
	else
	{
		...
		const float baseSpeed = (float)fsel(selChange*followCameraSettings.m_cameraSpeed,followCameraSettings.m_cameraSpeed,10000.0f);

		const Vec3 offsetSpeeds( 
									(float)fsel( followCameraSettings.m_offsetSpeeds.x-FLT_EPSILON, followCameraSettings.m_offsetSpeeds.x, baseSpeed),
									(float)fsel( followCameraSettings.m_offsetSpeeds.y-FLT_EPSILON, followCameraSettings.m_offsetSpeeds.y, baseSpeed),
									(float)fsel( followCameraSettings.m_offsetSpeeds.z-FLT_EPSILON, followCameraSettings.m_offsetSpeeds.z, baseSpeed));
	...
	viewParams.fov = (float)fsel( followCameraSettings.m_cameraFov, followCameraSettings.m_cameraFov, viewParams.fov );
	...
	case CActor::eASM_Killer:
		{
			...
			// Apply Post Effects.
			const float focalRange = (float)fsel( g_pGameCVars->g_killercam_dofFocusRange-minFocalRange, g_pGameCVars->g_killercam_dofFocusRange, minFocalRange );
			pRenderer->EF_SetPostEffectParam("Dof_User_Active", 1.f);
			pRenderer->EF_SetPostEffectParam("Dof_User_FocusDistance", focalDist);
			pRenderer->EF_SetPostEffectParam("Dof_User_FocusRange", focalRange);
			pRenderer->EF_SetPostEffectParam("Dof_User_BlurAmount", g_pGameCVars->g_killercam_dofBlurAmount);
		}
		break;
	}
...
void CDeathCameraMode::Activate(const CPlayer& clientPlayer)
{
	...
	m_timeOut = deathCam + ( (float)fsel(-killCam,0.f,0.25f) * (float)fsel(-deathCam,0.f,1.f) );
	m_invTimeMulti = fres((float)fsel(0.5f-m_timeOut,3.0f,m_timeOut-0.5f));
}
...
bool CDeathCameraMode::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	// do a test between the "safe" and "ideal" focus points to find the best pos that's not through a wall (in theory)
	{
		...
		if (!sphereDir.IsZero())
		{
			...
			float  hitDist = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(sphere.type, &sphere, sphereDir, (ent_static | ent_terrain | ent_rigid), &pContact, 0, (rwi_ignore_noncolliding | rwi_stop_at_pierceable), 0, nullptr, 0, !skipList.empty() ? &skipList[0] : NULL, skipList.size());
			if (hitDist > 0.f)
			{
	...
	// calculate the look pos and (desired) cam pos based on the type of look
	{
		...
		const float finalDist = (float)fsel((g_pGameCVars->g_tpdeathcam_camDistFromPlayerEnd)-g_pGameCVars->g_tpdeathcam_camDistFromPlayerMin,g_pGameCVars->g_tpdeathcam_camDistFromPlayerEnd,g_pGameCVars->g_tpdeathcam_camDistFromPlayerMin);
		...
		if (!m_firstUpdate)
		{
			camPos.z = (float)fsel(m_firstFocusPos.z-focusPos.z,m_firstFocusPos.z,camPos.z);
		}
	}
	...
	// do small reverse test towards the focus pos (from direction of the cam pos) to find a pos sufficiently clear of the ground for the main focus-to-cam test
	{
		...
		if (!testDir.IsZero())
		{
			...
			geom_contact*  pContact = NULL;
			float  hitDist = gEnv->pPhysicalWorld->PrimitiveWorldIntersection(sphere.type, &sphere, testDir, (ent_static | ent_terrain | ent_rigid), &pContact, 0, (rwi_ignore_noncolliding | rwi_stop_at_pierceable), 0, nullptr, 0, !skipList.empty() ? &skipList[0] : NULL, skipList.size());
	...
	bumpUp = (float)fsel(bumpUp-m_prevBumpUp,bumpUp,m_prevBumpUp);
...
bool CDeathCameraModeSinglePlayer::UpdateView(const CPlayer& clientPlayer, SViewParams& viewParams, float frameTime)
{
	...
	if (m_params.dof_enable)
	{
		...
		m_fFocusDistance += static_cast<float>(fsel(fDelta, 1.0f, -1.0f) * min(fabs_tpl(fDelta), (m_params.dofDistanceSpeed * frameTime)));

		// Approximate range
		fDelta = fDesiredFocusRange - m_fFocusRange;
		m_fFocusRange += static_cast<float>(fsel(fDelta, 1.0f, -1.0f) * min(fabs_tpl(fDelta), (m_params.dofRangeSpeed * frameTime)));

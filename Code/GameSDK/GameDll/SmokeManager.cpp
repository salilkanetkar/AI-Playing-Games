...
void CSmokeManager::UpdateSmokeInstance(SSmokeInstance& smokeInstance, float dt)
{
	...
	if(pGrenade)
	{
		...
		const float fSpeed = fDistanceTravelled * fres(dt);

		const float fNewRadius = smokeInstance.fCurrentRadius + (float)fsel( smokeInstance.fTimer-kInitialDelay, ((kSmokeRadiusIncrease - fSpeed) * dt), 0.0f);
		...
		{
			pe_params_pos pos;
			pos.scale = clamp_tpl(smokeInstance.fCurrentRadius * fres(kMaxSmokeRadius), 0.1f, 1.0f);
...
bool CSmokeManager::IsPointInSmoke(const Vec3& vPos, float& outInsideFactor) const
{
	...
	for(int i = 0; i < kNumActiveSmokeInstances; i++)
	{
		...
		{
			outInsideFactor = 1.0f - ((float)sqrt_fast_tpl(fDistanceSq) * (float)fres(sqrt_fast_tpl(fCurrentRadiusSq)));
			return true;
		}
	}

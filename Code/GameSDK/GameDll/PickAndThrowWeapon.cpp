...
DECLARE_DEFAULT_COMPONENT_FACTORY(CPickAndThrowWeapon,CPickAndThrowWeapon)
...
void CPickAndThrowWeapon::OnSelected(bool selected)
{
	...
	if (selected)
	{
		...
		if (IComponentRender *pRenderProxy = validTargetEntity ? (IComponentRender*)pTargetEntity->GetComponent<IComponentRender>().get() : NULL)
		{
...
void CPickAndThrowWeapon::SetUpCollisionPropertiesAndMassInfo( const bool pickedUp )
{
	...
	if (pickedUp)
	{
		...
		if (pObjectPhysics->GetStatus(&dynamics))
		{
			...
			m_objectMass = (float)fsel( maxMass-dynamics.mass, dynamics.mass, maxMass );
		}
...
void CPickAndThrowWeapon::RestoreEntityViewDist( const SInfiniteViewDistEntity& info ) const
{
	...
	if (pEntity)
	{
		...
		if (pRenderProxy)
		{
			...
			if (pRenderNode)
			{
				pRenderNode->SetViewDistanceMultiplier( info.oldViewDistRatio );
			}
		}
	}
}
...
void CPickAndThrowWeapon::ThrowObject()
{
	...
	if (pOwnerPlayer != NULL)
	{
		...
		if(m_state == eST_CHARGED_THROW_POST_RELEASE)
		{
			...
			IComponentPhysics* pPhysProxy = static_cast<IComponentPhysics*>(pEntity->GetComponent<IComponentPhysics>().get());
			if(pPhysProxy)
			{
				AABB aabb;
		...
		{							
			...
			if (m_state == eST_PUSHINGAWAY_POWER)
			{
				...
				if (pRenderNode)
				{
					...
					info.oldViewDistRatio = pRenderNode->GetViewDistanceMultiplier();
...
IEntity* CPickAndThrowWeapon::CalculateBestChargedThrowAutoAimTarget(const Matrix34& attackerTransform) const
{
		...
		if(pEntity && PickAndThrow::AllowedToTargetPlayer(GetOwnerId(),targetEntityId))
		{
			...
			if(toTarget.GetLengthSquared() < autoAimDistSqrd)
			{
				...
				if(PickAndThrow::TargetEntityWithinFrontalCone(attackerPos,targetTestPos, attackerFacing,coneSizeRads, theta))
				{
					...
					float targetDistScore    =  1.0f - ( toTarget.GetLength() * fres(max(g_pGameCVars->pl_pickAndThrow.chargedThrowAutoAimDistance, FLT_EPSILON)) ); 
					...
					const float targetConeScore		 =  1.0f - ( theta * fres(max(coneSizeRads*0.5f, FLT_EPSILON)) );
	...
	if(pBestTarget && g_pGameCVars->pl_pickAndThrow.chargedThrowDebugOutputEnabled)
	{
		...
		IComponentPhysics* pPhysProxy = static_cast<IComponentPhysics*>(pBestTarget->GetComponent<IComponentPhysics>().get());
		if(pPhysProxy)
		{
			AABB aabb;
			pPhysProxy->GetWorldBounds(aabb);
...
void CPickAndThrowWeapon::SerializeSpawnInfo( TSerialize ser, IEntity*ent )
{

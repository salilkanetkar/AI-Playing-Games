...
CGame::~CGame()
{
	m_pFramework->EndGameContext(true);
  ...
  LOADING_TIME_PROFILE_SECTION(GetISystem());
	...
	if (!gEnv->IsEditor())
	{
		...
		if(gEnv->IsDedicated())
		{
			...
			configFileName.Format("@ROOT@, %s.cfg", DEDICATED_BASE_CONFIG_NAME);
...
//		gEnv->pNetwork->SetMultithreadingMode(INetwork::NETWORK_MT_PRIORITY_NORMAL);
		...
		//FakeGetLobby()->SetUserPacketEnd(eGUPD_End);
		...
		if (!(g_pGameCVars && (g_pGameCVars->g_useOnlineServiceForDedicated) && gEnv->IsDedicated()) && FakeGetLobby())
		{
			error = FakeGetLobby()->Initialise(eCLS_LAN, features, CGameBrowser::ConfigurationCallback, CGameBrowser::InitialiseCallback, this);
		...
		if(!gEnv->IsDedicated())
		{
			error = 0;// FakeGetLobby()->Initialise(eCLS_Online, features, CGameBrowser::ConfigurationCallback, CGameBrowser::InitialiseCallback, this);
		}
		else
		{
	...
	{
        m_pLightningGameEffect = GAME_FX_SYSTEM.CreateEffect<GameSDK::CLightningGameEffect>();
  ...
  g_pGame->GetIGameFramework()->GetIVehicleSystem()->RegisterVehicleClient(pVehicleClient);
	...
	ICryLobby *pLobby = gEnv->pNetwork ? FakeGetLobby() : NULL;
	if (pLobby)
	{
	...
	INetwork *net= gEnv->pNetwork;
	GridMate::IGridMate* gridMate = net ? net->GetGridMate() : 0;

	if (net && 0)
	{
		GridMate::StartGridMateService<GridMate::LANSessionService>(gridMate, GridMate::SessionServiceDesc());

		// On LAN, we become the server
		GridMate::CarrierDesc carrierDesc;
		carrierDesc.m_connectionTimeoutMS = 10000;
		carrierDesc.m_threadUpdateTimeMS = 30;
		carrierDesc.m_port = gEnv->pConsole->GetCVar("cl_serverport")->GetIVal();   // use the port we were supposed to find the server at to host our session
		// so other clients can find the session without changing their configuration.
		carrierDesc.m_enableDisconnectDetection = true; // !!gEnv->pConsole->GetCVar("gm_disconnectDetection")->GetIVal();

		GridMate::LANSessionParams sp;
		sp.m_localMember = net->GetGridMate()->GetOnlineService()->GetUser();
		sp.m_port = carrierDesc.m_port + 1;
		...
		sp.m_params[sp.m_numParams].SetValue("localServer");
		sp.m_numParams++;
		sp.m_params[sp.m_numParams].m_id = "sv_map";
		sp.m_params[sp.m_numParams].SetValue("theMap");
		sp.m_numParams++;

		//net->HostSession(sp, carrierDesc);
		//net->FindSession(sp);
		//net->JoinSession(si);
	}
	...
	m_pPrefabManager = m_pFramework->GetIPrefabManager();
...
//		gEnv->pNetwork->SetMultithreadingMode(INetwork::NETWORK_MT_PRIORITY_HIGH);
		...
		if (!gEnv->IsDedicated())
		{
			...
			//FakeGetLobby()->Initialise(eCLS_Online, eCLSO_Voice, CGameBrowser::ConfigurationCallback, NULL, NULL);
		}
		
		const char* mpConfigName = NULL;

		EGameSDKPlatform platform = GetPlatform();
		switch (platform)
		{
			case ePlatform_PC:
			{
...
//		gEnv->pNetwork->SetMultithreadingMode(INetwork::NETWORK_MT_PRIORITY_NORMAL);
		...
		//FakeGetLobby()->Terminate(eCLS_Online, eCLSO_Voice, NULL, NULL);
	}
#endif
	...
	else
	{
		...
		if (!gEnv->IsEditor())
		{
			...
			//if( FakeGetLobby()->GetLobbyServiceType() == eCLS_Online )
			//{
	...
	gEnv->pRenderer->EF_SetPostEffectParam("HudSilhouettes_FillStr", hudSilFillStr);
}
...
EGameSDKPlatform CGame::GetPlatform() const
{
	EGameSDKPlatform platform = ePlatform_Unknown;
...
bool CGame::ReadProfilePlatform(XmlNodeRef &platformsNode, EGameSDKPlatform platformId)
{
	bool bResult = false;
...
void CGame::InitScriptBinds()
{
	...
	m_pScriptBindBoids = new GameSDK::CScriptBind_Boids(m_pFramework->GetISystem());
    ...
    m_pScriptBindLightningArc = new GameSDK::CScriptBind_LightningArc(m_pFramework->GetISystem());
...
void CGame::OnPlatformEvent(const IPlatformOS::SPlatformEvent& event)
{
	...
	{
#if 0
		// TODO: investigate why this storage class disappeared. Seems it will mater on some devices? Console?
...
#if 0
		// TODO: investigtate why this storage method disappeared. Seems we might need it.
...
void CGame::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
{
	switch (event)
	{
		...
		case ESYSTEM_EVENT_LEVEL_LOAD_START:
		{
			...
			/* The recording system is disabled due to a dependancy on spectator mode which doesn't appear to exist in GameSDK.
		...
		case ESYSTEM_EVENT_EDITOR_GAME_MODE_CHANGED:
			{
				if (wparam != 0)
				{
					...
					{
						IComponentAudio* const pIComponentAudio = (IComponentAudio*)pLocalPlayerEntity->GetComponent<IComponentAudio>().get();

						if (pIComponentAudio != NULL)
						{
							pIComponentAudio->SetCurrentEnvironments(INVALID_AUDIO_PROXY_ID);
						}
					}
				}
...
IGame::ExportFilesInfo CGame::ExportLevelData( const char* levelName, const char* missionName ) const
{
	...
	{
		IGame::ExportFilesInfo::GetNameForFile( baseFileName, exportedFileCount, fileName , 512);
...
void CGame::LoadExportedLevelData( const char* levelName, const char* missionName )
{
	...
	{
		IGame::ExportFilesInfo::GetNameForFile( baseFileName, loadedFileCount, fileName, 512 );

...
bool CPlayerStateLadder::OnPrePhysicsUpdate( CPlayer& player, const SActorFrameMovementParams &movement, float frameTime )
{
	...
	else if (player.GetActorStats())
	{
			...
			m_fractionBetweenRungs += frameTime * m_climbInertia * (float)fsel (m_climbInertia, movementSpeedUpwards, movementSpeedDownwards) + clamp_tpl(settleOffset, -maxAutoSettleMovement, maxAutoSettleMovement);

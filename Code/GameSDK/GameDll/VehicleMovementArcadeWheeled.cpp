...
void CVehicleMovementArcadeWheeled::UpdateSounds(const float deltaTime)
{
	...
	if (m_isEnginePowered && !m_isEngineGoingOff)
	{
		...
		m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleRPM], m_rpmScale);
	...
	m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleSlip], m_movementInfo.skidValue);
...
void CVehicleMovementArcadeWheeled::UpdateSuspension(const float deltaTime)
{
	...
	if ((fabsf(diffSusp) + fabsf(diffStabi)) != 0.f) // if (diffSusp || diffStabi)
	{
		...
		{
			float maxSpeed = (float)fsel(-m_pSharedParams->suspDampingMaxSpeed, 0.15f*m_maxSpeed, m_pSharedParams->suspDampingMaxSpeed);
			...
			if (m_brakeTimer > 0.f)
			{
				...
				{ 
					if (m_brakeTimer > m_pSharedParams->soundParams.airbrakeTime)
					{
						...
						m_pIComponentAudio->PlaySound(name, Vec3(0), FORWARD_DIRECTION, FLAG_SOUND_DEFAULT_3D, 0, eSoundSemantic_Vehicle);                
					}          
				}*/
			}
...
void CVehicleMovementArcadeWheeled::UpdateSuspensionSound(const float deltaTime)
{
	...
	{ 
		...
		{	
			...
			m_pIComponentAudio->SetRtpcValue(m_audioControlIDs[eSID_VehicleStroke], fStroke);
...
float CVehicleMovementArcadeWheeled::CalcSteering(float steer, float speedRel, float rotateYaw, float dt)
{
	...
	steerTarget = (float)fsel(fabsf(steerTarget)-0.01f, steerTarget, 0.f);
	float steerError = steerTarget - steer;
	steerError = (float)fsel(fabsf(steerError)-0.01f, steerError, 0.f);
	...
	if ((steerError*steer) < 0.f)
	{
		...
		dt = (float)fsel(absSteer, 0.f, absSteer/DEG2RAD(m_pSharedParams->steerRelaxation)); // Calculate any overshoot in dt
		steer = (float)fsel(absSteer, (float)fsel(steer, +absSteer, -absSteer), 0.f);
...
void CVehicleMovementArcadeWheeled::InternalPhysicsTick(float dt)
{
	...
	{ 
		...
		if (ws->suspLenFull != 0)
		{
			...
			extension = (float)fsel(extension, 0.f, extension);
	...
	topSpeed = (float)fsel(speed, topSpeed, m_pSharedParams->handling.reverseSpeed);
	topSpeed *= m_topSpeedFactor;
			...
			else 
			{
				...
				else
				{
					if (canDeccelerate)
					{
						...
						w->w = (float)fsel(fabsf(w->w)-fabsf(dw), w->w-dw, 0.f);
					}
					w->w *= 0.9f;
				}
				w->lastW = w->w;
...
void CVehicleMovementArcadeWheeled::TickGears(float dt, float averageWheelSpeed, float throttle, float forwardSpeed, float contact)
{
	...
	{
		const bool isTank = m_pSharedParams->tankHandling.additionalSteeringAtMaxSpeed > 0.f;
		const float topSpeed = (float)fsel(forwardSpeed, m_pSharedParams->handling.topSpeed, m_pSharedParams->handling.reverseSpeed) * m_topSpeedFactor;
		const float invTopSpeed = 1.f/topSpeed;
		...
		//////////////////////////////////////
		const float interpSpeed = (float)fsel(fabsf(throttle)-0.05f, m_pSharedParams->gears.rpmInterpSpeed, m_pSharedParams->gears.rpmRelaxSpeed);
		...
		float loadInterpSpeed = (float)fsel(totalRpmLoad - m_gears.rpmLoad, m_pSharedParams->gears.rpmLoadChangeSpeedUp, m_pSharedParams->gears.rpmLoadChangeSpeedDown);
		...
		if (matId != emitterIt->matId)
		{
			...
			if (pSurfaceType)
			{
				...
				if (surfaceTypeName.find("mat_") != std::string::npos)
				{
					...
					{
						m_pIComponentAudio->SetSwitchState(m_audioControlIDs[eSID_VehicleSurface], nSurfaceStateID);
					}
				}
			}
		}

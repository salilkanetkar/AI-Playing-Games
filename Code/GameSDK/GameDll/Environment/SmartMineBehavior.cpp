...
private:
	...
	IComponentTrigger* GetTriggerProxyForSmartMine( CSmartMine& smartMine ) const;
...
void CSmartMineBehavior::SetupSmartMine( CSmartMine& smartMine, const SmartMineSetupProperties& smartMineProperties )
{
	...
	// Other proxies
	IComponentTrigger* pTriggerProxy = smartMine.GetEntity()->GetOrCreateComponent<IComponentTrigger>().get();
	smartMine.GetEntity()->GetOrCreateComponent<IComponentAudio>();
	
	//IComponentTrigger* pTriggerProxy = GetTriggerProxyForSmartMine( smartMine );
...
IComponentTrigger* CSmartMineBehavior::GetTriggerProxyForSmartMine( CSmartMine& smartMine ) const
{
	return static_cast<IComponentTrigger*>(smartMine.GetEntity()->GetComponent<IComponentTrigger>().get());
}
...
void CSmartMineBehavior::UpdateVisualState( CSmartMine& smartMine, const EVisualState visualState )
{
	...
	if (pLightSource != NULL)
	{
		...
		const float specularMultiplier = (float)fsel( -properties.diffuseMultiplier, properties.specularMultiplier, (properties.specularMultiplier / (properties.diffuseMultiplier + FLT_EPSILON)));

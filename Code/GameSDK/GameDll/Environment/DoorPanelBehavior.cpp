...
void CDoorPanelBehavior::SetupDoorPanel( CDoorPanel& doorPanel )
{
	...
	// Other proxies
	pEntity->GetOrCreateComponent<IComponentAudio>();
}
...
bool CDoorPanelBehavior::SetupFlash( CDoorPanel& doorPanel )
{
	...
	if (pOriginalMaterial)
	{
		...
		else
		{
			...
			{
				IComponentRender* pEntityToShareRenderProxy = static_cast<IComponentRender*>(pEntityToShareScreen->GetComponent<IComponentRender>().get());

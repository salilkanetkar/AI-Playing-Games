...
void CRecoil::RecoilShoot(bool firstShot, float maxRecoil)
{
	...
	attack = (float)fsel(attack, attack, m_recoilParams.attack);
	m_attack = attack;
...
void CRecoil::UpdateRecoil(float recoilScale, float maxRecoil, bool weaponFired, bool weaponFiring, float frameTime)
{
	...
	const float frameDecay = (float)fsel(-decay, 0.0f, frameTime * recoilScale * maxRecoil * fres(decay+FLT_EPSILON));
	m_recoil = clamp_tpl(m_recoil - frameDecay, 0.0f, maxRecoil);

	const float t = (float)fsel(-maxRecoil, 0.0f, m_recoil * fres(maxRecoil+FLT_EPSILON));
...
void CRecoil::UpdateSpread(bool weaponFired, bool weaponFiring, float frameTime)
{
	...
	const float frameDecay = (float)fsel(-decay, 0.0f, ((m_spreadParams.max - m_spreadParams.min) * fres(decay + FLT_EPSILON)) * frameTime);
...
float CRecoil::GetMaxRecoil() const
{
	float scale = 1.0f;
	if (IsHoldingBreath())
		scale = (float)fsel(-m_recoilParams.recoil_holdBreathActive_m, 1.0f, m_recoilParams.recoil_holdBreathActive_m);
...
void CRecoil::PatchSpreadMod(const SSpreadModParams& spreadMod, const SSpreadParams& originalSpreadParams, float modMultiplier)
{
	...
	if(oldSpreadRange)
	{
		float inverseOldSpreadRange = fres(oldSpreadRange);

...
{
public:
	CTurretSoundManagerHelper( IComponentAudioPtr pIComponentAudio )
		: m_pIComponentAudio( pIComponentAudio )
	{
		assert( m_pIComponentAudio != NULL );
	}
	...
	void StopAllSounds()
	{
		//m_pIComponentAudio->StopAllSounds();
	...
	void StopSound( const size_t soundId )
	{
		/*assert( soundId < eTS_SoundCount );
		STurretSoundInfo& soundInfo = m_sounds[ soundId ];
		m_pIComponentAudio->StopSound( soundInfo.soundId );
		soundInfo.soundId = INVALID_SOUNDID;*/
	}
	...
	void PlaySound( const size_t soundId )
	{
		...
		if ( validSoundName )
		{
			...
			soundInfo.soundId = m_pIComponentAudio->PlaySound( soundName, offset, direction, FLAG_SOUND_DEFAULT_3D, 0, eSoundSemantic_Mechanic_Entity );*/
			soundInfo.speedParamValue = FLT_MAX;
		}
	}
	...
	void SetSoundParam( const size_t soundId, const char* const paramName, const float value )
	{
		assert( soundId < eTS_SoundCount );
		const STurretSoundInfo& soundInfo = m_sounds[ soundId ];
		/*_smart_ptr< ISound > pSound = m_pIComponentAudio->GetSound( soundInfo.soundId );
		if ( pSound )
		{
...
private:
	IComponentAudioPtr m_pIComponentAudio;
...
{
public:
	CTurretSoundManager( SmartScriptTable pSoundPropertiesTable, ICharacterInstance* pCharacterInstance, IComponentAudioPtr pIComponentAudio )
		: m_soundManagerHelper( pIComponentAudio )
		, m_oldYawRadians( 0 )
		, m_oldPitchRadians( 0 )
